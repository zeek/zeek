...
Luckily, this flipping usually happened before the first packet of connection
is processed. More recently, however, flipping on
`the second packet <https://github.com/zeek/zeek/pull/2191>` has been added.
...
originator and responder notion. And always sort these deterministically and
name them endpoints ``left`` and ``right``. The notion of originator and responder
wouldn't vanish from Zeek, but implemented on a higher-level instead. For example,
the ``IPBasedConnKey`` class holds a ``flipped`` member and received a ``FlipRoles()`` API.
However, it actually seems unreasonable that on the raw connection tracking layer
the originator and responder concept should even exist. They make things hard.
