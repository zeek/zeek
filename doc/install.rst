...
We provide official Docker images on Docker Hub at https://hub.docker.com/u/zeek
    ...
    * For the latest release in a given series: ``docker pull zeek/zeek:8.1``
    * For a specific release: ``docker pull zeek/zeek:8.1.1``
...
For example, for the latest Zeek 8.0 LTS release on Ubuntu 24.04 the steps look as follows:
...
.. note:: Our motivation for this approach is twofold. First, it guarantees LTS
   ...
   out. For example, when you install the ``zeek-7.0`` packages, you will not
   end up on Zeek 8.0 until you decide to switch. Second, it reflects the fact
