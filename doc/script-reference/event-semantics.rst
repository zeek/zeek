...
handler has a zeekTODO:attr:`&group` attribute. The value of the group
...
function from the :ref:`configuration framework\`
<framework-configuration>` to enable and disable the
``http-print-debugging`` event group at runtime.
...
zeekTODO:see:`enable_event_group` is invoked to ensure the
``http-print-debugging`` group is enabled. Conversely, when the option
is set to ``F``, zeekTODO:see:`disable_event_group` disables all event
...
functions zeekTODO:see:`disable_module_events` and
zeekTODO:see:`enable_module_events` can be used to disable and enable
...
.. note::
   ...
   based on the zeekTODO:see:`ssl_extension` events from SSL ClientHello
   ...
   While all zeekTODO:see:`ssl_extension` handlers are processed jointly
