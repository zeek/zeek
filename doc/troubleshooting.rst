...
Script profiling (--profile-scripts)
====================================

Zeek provides a script profiling facility that can be used to understand where time is spent executing Zeek scripts during runtime. This can be useful when diagnosing performance issues related to script execution.

Script profiling can be enabled by starting Zeek with the ``--profile-scripts`` command-line option. When enabled, Zeek periodically records profiling information about script execution and writes the results to profiling output files.

The profiling output is intended to provide a high-level view of script execution costs and can help identify scripts or script components that may contribute disproportionately to runtime overhead. For detailed interpretation of the output format and fields, users should consult the profiling output files generated by Zeek.
