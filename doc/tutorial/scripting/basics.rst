...
The body of our event does two things:
...
This uses :zeek:see:`sleep` in order to put some time between two calls.
...
Analyzers can add state to this record in order to piece together what
it needs, like how the HTTP analyzer correlates requests and responses
...
For more information, see :zeek:see:`int`, :zeek:see:`count`, :zeek:see:`bool`,
:zeek:see:`string`, and :zeek:see:`pattern`.
...
:zeek:see:`redef` to do far more. Here, we will use it to demonstrate one
of the most powerful features in Zeek: redefining the ``connection``
record.
...
network traffic, it triggers the :zeek:see:`http_request` event. The HTTP
analyzer may then use that event in order to gather data, correlate a
