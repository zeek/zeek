...
@if ( Cluster::is_enabled() && Cluster::local_node_type() != Cluster::MANAGER )
		...
		if ( batch_period == 0sec || batch_max_size == 0 ||
			(|gbatch$infos| == 0 && (now - gbatch$ts) >= batch_period) )
			{
		...
		if ( |gbatch$infos| == 1 && batch_max_size > 1 && ! gbatch$timer_pending )
			{
			...
			schedule batch_period { suppression_batch_timer() };
		...
		if ( |gbatch$infos| == batch_max_size )
			gbatch_send_and_reset();
@endif
		}
	}
