...
export {
	...
	## logging. If a string reaches this limit, the log output will be truncated.
	## Setting this to zero disables the limiting.
	const limit_field_string_bytes = 4096 &redef;

	## The maximum number of elements any container can hold when logging.  If a
	## container reaches this limit, the log output will be truncated.  Setting this
	## to zero disables the limiting.
	const limit_field_container_elements = 100 &redef;
	...
	const limit_total_string_bytes = 256000 &redef;

	## The total number of container elements a record may log. This is the sum of all
	## container elements logged for the record. If this limit is reached, all further
	## containers will be logged as empty containers. If the limit is reached while
	## processing a container, the container will be truncated in the output.  Setting
	## this to zero disables the limiting.
	const limit_total_container_elements = 500 &redef;
}
