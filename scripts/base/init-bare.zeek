...
export {
	type Interface: record {
		# The link type for the interface.
		link_type: count;

		snaplen: count;
		# The name of the device used to capture data.
		name: string &optional;
		# A description of the device used to capture data.
		description: string &optional;
		# A list of IPv4 addresses assigned to the interface.
		ipv4_addrs: vector of string &optional;
		# A list of IPv6 addresses assigned to the interface.
		ipv6_addrs: vector of string &optional;
		# The MAC address of the device used to capture data.
		mac_addr: string &optional;
		# The EUI-64 address of the device used to capture data.
		eui_addr: string &optional;
		# The interface speed, in bits per second.
		speed: count &optional;
		# The resolution of timestamps, in fractions of a second. For example,
		# microsecond resolution will be 1e6.
		ts_resolution: count &optional;
		# The filter used to capture traffic. This is not the BPF filter passed to
		# the Zeek IO source, but the filter used to originally capture the
		# traffic in the pcapng file.
		filter: string &optional;
		# The operating system of the machine in which this interface is
		# installed.
		os: string &optional;
		# The length of the Frame Check Sequence for this interface, in bits.
		fcs_len: count &optional;
		# The offset in seconds that must be added to the timestamp of each packet
		# to obtain the absolute timestamp of a packet.
		tsoffset: count &optional;
		# A description of the interface hardware.
		hardware: string &optional;
		# The transmit speed of the interface, in bits per second.
		txspeed: count &optional;
		# The receive speed of the interface, in bits per second.
		rxspeed: count &optional;
		# The IANA timezone name for the timezone where the interface is located.
		iana_tzname: string &optional;
	};

	type PacketOptions: record {
		# A vvector of comments that can be passed along with each packet.
		comments: vector of string &optional;
		# A set of flags that denote various features of the packet.
		# TODO: This should be broken into the various flag values.
		flags: count &optional;
		# A vector of hashes of the packet. Each string is a name of the hash
		# algorithm used followed by the hash value, separated by a colon. See
		# Section 4.3 of the Pcapng for a list of the valid hash algorithms.
		hash: vector of string &optional;
		# The number of packets dropped by the interface and/or operating system
		# between this packet and the preceding one.
		dropcount: count &optional;
		# A unique identifier for the packet. This can be used to track duplicate
		# packets being seen by multiple interfaces.
		packet_id: count &optional;
		# The identifier of which queue on the interface the packet was received.
		queue: count &optional;
		# A vector of verdicts of the packet, which indicate what would be done
		# with the packet after processing it. Each packet is a verdict type
		# followed by the verdict, separated by a colon.  See Section 4.3 of the
		# Pcapng for a list of the valid verdict types.
		verdict: vector of string &optional;
		# An string containing the process ID and thread ID which originated the
		# packet, separated by a dash.
		processid_threadid: string &optional;
	};

	type FileInfo: record {
		## The version of the file, in major.minor format.
		version: string;
		## A vector of 'comment' fields. Matches the 'opt_comment' option from the
		## Section Header Block.
		comment: vector of string &optional;
		## The hardware description for the file. Matches the 'shb_hardware'
		## option from the Section Header Block.
		hw_desc: string &optional;
		## The operating system description for the file. Matches the 'shb_os'
		## option from the Section Header Block.
		os_desc: string &optional;
		## The user application description for the file. Matches the
