...
# send a ZeroMQ::hello() event to the topic, announcing the presence of this
# node to the one that created the subscription. This goes in both directions:
# The other node will see the subscription incoming from existing or new nodes
# and publish ZeroMQ::hello() as well. In other words, every node says hello
# to all other nodes.
event Cluster::Backend::ZeroMQ::subscription(topic: string)
	...
	# If we saw a ZeroMQ::hello from the other node, send
	# it a Cluster::hello.
...
# Receiving ZeroMQ::hello() from another node, if we received a
# subscription for the node id's topic, reply with Cluster::hello.
# Also, if we never saw the node go away, log a warning and raise
# a Cluster::node_down() now.
event Cluster::Backend::ZeroMQ::hello(name: string, id: string)
	...
	# We can only publish here if the other system *also* has a subscription
	# setup, otherwise the other node will not see the Cluster::hello()
