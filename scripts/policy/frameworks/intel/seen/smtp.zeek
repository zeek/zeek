	...
	if ( c?$smtp )
		...
		if ( c$smtp?$to )
			...
			for ( email_to_addr in c$smtp$to )
				{
				Intel::seen(Intel::Seen(
					$indicator=extract_first_email_addr(email_to_addr),
					$indicator_type=Intel::EMAIL,
					$conn=c,
					$where=SMTP::IN_TO));
				}
