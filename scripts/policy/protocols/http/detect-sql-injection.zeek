##! A revised form of SQL injection attack detection in HTTP.
...
event zeek_init() &priority=3
	...
	local r1 = SumStats::Reducer($stream="http.sqli.attacker",
				$apply=set(SumStats::SUM, SumStats::SAMPLE),
				$num_samples=1);
	local r2 = SumStats::Reducer($stream="http.sqli.victim",
				$apply=set(SumStats::SUM, SumStats::SAMPLE),
				$num_samples=1);
