...
event zeek_init()
	...
	min_lengths_tls11 += Heartbleed::min_length($cipher=/_AES_256_GCM_SHA384$/, $min_length=43);
	min_lengths_tls11 += Heartbleed::min_length($cipher=/_AES_128_GCM_SHA256$/, $min_length=43);
	min_lengths_tls11 += Heartbleed::min_length($cipher=/_256_CBC_SHA384$/, $min_length=96);
	min_lengths_tls11 += Heartbleed::min_length($cipher=/_256_CBC_SHA256$/, $min_length=80);
	min_lengths_tls11 += Heartbleed::min_length($cipher=/_256_CBC_SHA$/, $min_length=64);
	min_lengths_tls11 += Heartbleed::min_length($cipher=/_128_CBC_SHA256$/, $min_length=80);
	min_lengths_tls11 += Heartbleed::min_length($cipher=/_128_CBC_SHA$/, $min_length=64);
	min_lengths_tls11 += Heartbleed::min_length($cipher=/_3DES_EDE_CBC_SHA$/, $min_length=48);
	min_lengths_tls11 += Heartbleed::min_length($cipher=/_SEED_CBC_SHA$/, $min_length=64);
	min_lengths_tls11 += Heartbleed::min_length($cipher=/_IDEA_CBC_SHA$/, $min_length=48);
	min_lengths_tls11 += Heartbleed::min_length($cipher=/_DES_CBC_SHA$/, $min_length=48);
	min_lengths_tls11 += Heartbleed::min_length($cipher=/_DES40_CBC_SHA$/, $min_length=48);
	min_lengths_tls11 += Heartbleed::min_length($cipher=/_RC4_128_SHA$/, $min_length=39);
	min_lengths_tls11 += Heartbleed::min_length($cipher=/_RC4_128_MD5$/, $min_length=35);
	min_lengths_tls11 += Heartbleed::min_length($cipher=/_RC4_40_MD5$/, $min_length=35);
	min_lengths_tls11 += Heartbleed::min_length($cipher=/_RC2_CBC_40_MD5$/, $min_length=48);
	min_lengths += Heartbleed::min_length($cipher=/_256_CBC_SHA$/, $min_length=48);
	min_lengths += Heartbleed::min_length($cipher=/_128_CBC_SHA$/, $min_length=48);
	min_lengths += Heartbleed::min_length($cipher=/_3DES_EDE_CBC_SHA$/, $min_length=40);
	min_lengths += Heartbleed::min_length($cipher=/_SEED_CBC_SHA$/, $min_length=48);
	min_lengths += Heartbleed::min_length($cipher=/_IDEA_CBC_SHA$/, $min_length=40);
	min_lengths += Heartbleed::min_length($cipher=/_DES_CBC_SHA$/, $min_length=40);
	min_lengths += Heartbleed::min_length($cipher=/_DES40_CBC_SHA$/, $min_length=40);
	min_lengths += Heartbleed::min_length($cipher=/_RC4_128_SHA$/, $min_length=39);
	min_lengths += Heartbleed::min_length($cipher=/_RC4_128_MD5$/, $min_length=35);
	min_lengths += Heartbleed::min_length($cipher=/_RC4_40_MD5$/, $min_length=35);
	min_lengths += Heartbleed::min_length($cipher=/_RC2_CBC_40_MD5$/, $min_length=40);
	}
...
event ssl_heartbeat(c: connection, is_client: bool, length: count, heartbeat_type: count, payload_length: count, payload: string)
	...
	if ( heartbeat_type == 1 )
		...
		if ( payload_length > checklength )
			...
			NOTICE(Notice::Info(
				$note=Heartbleed::SSL_Heartbeat_Attack,
...
event ssl_encrypted_heartbeat(c: connection, is_client: bool, length: count)
	...
	if ( c$ssl$originator_heartbeats > c$ssl$responder_heartbeats + 3 )
			NOTICE(Notice::Info(
				$note=SSL_Heartbeat_Many_Requests,
	...
	if ( is_client )
		...
		if ( c$ssl?$last_originator_heartbeat_request_size && c$ssl$last_originator_heartbeat_request_size < length )
			{
			NOTICE(Notice::Info(
				$note=SSL_Heartbeat_Attack_Success,
				$msg=fmt(
					"An encrypted TLS heartbleed attack was probably detected! First packet client record length %d, first packet server record length %d. Time: %f",
					c$ssl$last_originator_heartbeat_request_size,
					length,
					duration),
				$conn=c,
