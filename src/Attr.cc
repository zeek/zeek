...
void Attributes::CheckAttr(Attr* a)
	...
	case ATTR_EXPIRE_FUNC:
		...
		const type_list* index_types = the_table->IndexTypes();

		///// HACK FOR BACKWARDS COMPATABILITY /////
		// before expire_func could take multiple //
		// args, accepting i: any and then doing  //
		// tuple unpacking would get around it.   //
		////////////////////////////////////////////
		if (e_ft->Args()->NumFields() == 2)
			{
			if ((*e_ft->ArgTypes()->Types())[1]->Tag() == TYPE_ANY)
				break;
			}

		type_list expected_args;
		expected_args.push_back(type->AsTableType());
		for (const auto& t : *index_types)
			{
