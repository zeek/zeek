...
public:
	...
	void run()
		...
		assert(res == 0);  // 
		// TODO: Generate thread ID in more platform-independent way (e.g. manual 4-byte copy into unsigned int).
		// unsigned int id = (unsigned int)(pthread_self());
		while(!thread_finished)
			{
			MessageEvent *msg = in_queue.get();
			// Might be good to move this into REALLY_REALLY_REALLY_VERBOSE_DBG_THREADS or something.
