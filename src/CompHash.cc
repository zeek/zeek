			...
			switch ( v->GetType()->Tag() )
				...
				case TYPE_RECORD:
					...
					for ( int i = 0; i < num_fields; ++i )
						...
						if ( ! (kp = SingleValHash(type_check, kp, rt->GetFieldType(i).get(), rv_i,
						                           optional)) )
							return nullptr;
						}
					...
					struct HashKeyComparer
						...
						bool operator()(const HashKey* a, const HashKey* b) const
							...
							return strncmp(static_cast<const char*>(a->Key()),
							               static_cast<const char*>(b->Key()), a->Size()) < 0;
							}
						};
				...
				case TYPE_LIST:
					...
					*kp = lv->Length();
					...
					for ( int i = 0; i < lv->Length(); ++i )
						...
						Val* v = lv->Idx(i).get();
						if ( ! (kp1 =
						            SingleValHash(type_check, kp1, v->GetType().get(), v, false)) )
							return nullptr;
						}
					}
					break;
			...
			switch ( bt->Tag() )
				...
				case TYPE_RECORD:
					...
					for ( int i = 0; i < num_fields; ++i )
						...
						sz = SingleTypeKeySize(rt->GetFieldType(i).get(),
						                       rv ? rv->GetField(i).get() : nullptr, type_check, sz,
						                       optional, calc_static_size);
						if ( ! sz )
							return 0;
						}
				...
				case TYPE_TABLE:
					...
					for ( int i = 0; i < tv->Size(); ++i )
						...
						sz = SingleTypeKeySize(key->GetType().get(), key.get(), type_check, sz,
				...
				case TYPE_VECTOR:
					...
					for ( unsigned int i = 0; i < vv->Size(); ++i )
						...
						sz = SizeAlign(sz, sizeof(unsigned int));
						if ( val )
							sz = SingleTypeKeySize(bt->AsVectorType()->Yield().get(), val.get(),
			...
			switch ( t->Tag() )
				...
				case TYPE_RECORD:
					...
					for ( i = 0; i < num_fields; ++i )
						...
						kp = RecoverOneVal(k, kp, k_end, rt->GetFieldType(i).get(), &v, optional);

						// An earlier call to reporter->InternalError would have called abort() and
						// broken the call tree that clang-tidy is relying on to get the error
						// described. NOLINTNEXTLINE(clang-analyzer-core.uninitialized.Branch)
						if ( ! (v || optional) )
							{
							reporter->InternalError(
				...
				case TYPE_TABLE:
					...
					for ( int i = 0; i < n; ++i )
						...
						if ( t->IsSet() )
							...
							{
							ValPtr value;
							kp1 = RecoverOneVal(k, kp1, k_end, tt->Yield().get(), &value, false);
				...
				case TYPE_VECTOR:
					...
					for ( unsigned int i = 0; i < n; ++i )
						...
						if ( have_val )
							kp1 = RecoverOneVal(k, kp1, k_end, vt->Yield().get(), &value, false);
