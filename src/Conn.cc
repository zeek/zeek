...
void ConnectionRecordValCallback::Init(const Connection* arg_conn, RecordVal* arg_conn_val) {
    conn = arg_conn;
...
void ConnectionRecordValCallback::Done() {
    if ( ! conn_val ) {
        assert(conn == nullptr);
        return;
    }
...
const RecordValPtr& Connection::GetVal() {
    if ( ! conn_val ) {
        ...
        conn_val_cb.Init(this, conn_val.get());
