...
public:
	...
	[[deprecated("Remove in v4.1.  Use GetFunc().")]] Func* LocalHandler() { return local.get(); }

	const FuncTypePtr& GetType(bool check_export = true);

	[[deprecated("Remove in v4.1.  Use GetType().")]] FuncType* FType(bool check_export = true)
		{
		return GetType().get();
		}
	...
	[[deprecated("Remove in v4.1.  Use SetFunc().")]] void SetLocalHandler(Func* f);
...
using EventHandlerPtr [[deprecated("Remove in v4.1. Use zeek::EventHandlerPtr.")]] =
	zeek::EventHandlerPtr;
