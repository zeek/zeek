...
HashKey::HashKey(int copy_key, void* arg_key, uint64_t arg_size)
	{
	size = arg_size;
	is_our_dynamic = 1;
...
HashKey::HashKey(const void* arg_key, uint64_t arg_size, hash_t arg_hash)
	{
	size = arg_size;
	hash = arg_hash;
...
HashKey::HashKey(const void* bytes, uint64_t arg_size)
	{
	size = arg_size;
...
void* HashKey::CopyKey(const void* k, uint64_t s) const
	{
...
hash_t HashKey::HashBytes(const void* bytes, uint64_t size)
	{
