...
void KeyedHash::InitializeHmacMd5Seed() {
    if ( ! seeds_initialized )
        reporter->FatalError("InitializeSeeds() must be called before InitializeHmacMD5Seed()");

    if ( hmac_md5_seed_initialized )
        return;
    ...
    hmac_md5_seed_initialized = true;
}
