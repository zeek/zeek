...
static VectorValPtr BuildOptionsVal(const u_char* data, int len)
	...
	while ( len > 0 )
		...
		if ( opt->ip6o_type == 0 )
			...
			rv->Assign(2, make_intrusive<StringVal>(new String(data + off, opt->ip6o_len, true)));
	...
	switch ( type )
		...
		case IPPROTO_AH:
			...
			if ( Length() >= 12 )
				...
				rv->Assign(5,
				           make_intrusive<StringVal>(new String(data + off, Length() - off, true)));
				}
			}
			break;
		...
		case IPPROTO_MOBILITY:
			...
			switch ( mob->ip6mob_type )
				...
				case 1:
					...
					m->Assign(0, val_mgr->Count(ntohs(*((uint16_t*)msg_data))));
					m->Assign(1,
					          val_mgr->Count(ntohll(*((uint64_t*)(msg_data + sizeof(uint16_t))))));
				...
				case 3:
					...
					m->Assign(1,
					          val_mgr->Count(ntohll(*((uint64_t*)(msg_data + sizeof(uint16_t))))));
					m->Assign(2, val_mgr->Count(ntohll(*((uint64_t*)(msg_data + sizeof(uint16_t) +
					                                                 sizeof(uint64_t))))));
				...
				case 5:
					...
					m->Assign(0, val_mgr->Count(ntohs(*((uint16_t*)msg_data))));
					m->Assign(1, val_mgr->Bool(ntohs(*((uint16_t*)(msg_data + sizeof(uint16_t)))) &
					                           0x8000));
					m->Assign(2, val_mgr->Bool(ntohs(*((uint16_t*)(msg_data + sizeof(uint16_t)))) &
					                           0x4000));
					m->Assign(3, val_mgr->Bool(ntohs(*((uint16_t*)(msg_data + sizeof(uint16_t)))) &
					                           0x2000));
					m->Assign(4, val_mgr->Bool(ntohs(*((uint16_t*)(msg_data + sizeof(uint16_t)))) &
					                           0x1000));
					m->Assign(
						5, val_mgr->Count(ntohs(*((uint16_t*)(msg_data + 2 * sizeof(uint16_t))))));
				...
				case 6:
					...
					m->Assign(2,
					          val_mgr->Count(ntohs(*((uint16_t*)(msg_data + sizeof(uint16_t))))));
					m->Assign(
						3, val_mgr->Count(ntohs(*((uint16_t*)(msg_data + 2 * sizeof(uint16_t))))));
					off += 3 * sizeof(uint16_t);
