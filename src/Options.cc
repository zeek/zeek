...
void usage(const char* prog, int code)
	...
	fprintf(stderr,
	        ...
	        util::zeekenv("ZEEK_DNS_RESOLVER")
	            ? util::zeekenv("ZEEK_DNS_RESOLVER")
...
static void set_analysis_option(const char* opt, Options& opts)
	...
	if ( util::streq(opt, "inline") )
		...
		opts.analysis_options.inliner = opts.analysis_options.report_recursive = true;
...
Options parse_cmdline(int argc, char** argv)
	...
	char opts[256];
	util::safe_strncpy(opts, "B:e:f:G:H:I:i:j::n:O:p:r:s:T:t:U:w:X:CDFNPQSWabdhv", sizeof(opts));
