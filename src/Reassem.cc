...
DataBlock::DataBlock(DataBlockListInfo* list_info, const u_char* data,
...
void Reassembler::NewBlock(double t, uint64 seq, uint64 len, const u_char* data)
	...
	if ( ! blocks )
		blocks = last_block = start_block =
			new DataBlock(&block_list_info, data, len, seq, 0, 0, rtype);
	else
...
DataBlock* Reassembler::AddAndCheck(DataBlock* b, uint64 seq, uint64 upper,
	...
	if ( last_block && seq == last_block->upper )
		{
		last_block = new DataBlock(&block_list_info, data, upper - seq,
	...
	if ( upper <= b->seq )
		...
		new_b = new DataBlock(&block_list_info, data, upper - seq, seq,
		                      b->prev, b, rtype);
		if ( b == blocks )
			blocks = new_b;
		return new_b;
	...
	if ( seq < b->seq )
		...
		new_b = new DataBlock(&block_list_info, data, prefix_len, seq,
		                      b->prev, b, rtype);
		if ( b == blocks )
			blocks = new_b;
