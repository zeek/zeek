...
void ProfileLogger::Log()
	...
	*/
	...
	file->Write(fmt("%.06f Triggers: current=%" PRIu64 "total=%" PRIu64 " pending_total=%" PRIu64 " pending_completion=%" PRIu64 "\n",
			network_time, tstats.current, tstats.total, tstats.pending_all, tstats.pending_completion));
