...
static string container_type_name(const Type* ft) {
    ...
    if ( ft->Tag() == TYPE_RECORD )
        ...
        const auto& tl = (dynamic_cast<const IndexType*>(ft))->GetIndexTypes();
...
bool same_type(const Type& arg_t1, const Type& arg_t2, bool is_init, bool match_record_field_names) {
    ...
    switch ( t1->Tag() ) {
        ...
        case TYPE_OPAQUE: {
            const OpaqueType* ot1 = dynamic_cast<const OpaqueType*>(t1);
            const OpaqueType* ot2 = dynamic_cast<const OpaqueType*>(t2);
            return ot1->Name() == ot2->Name();
        }

        case TYPE_TABLE: {
            const IndexType* it1 = dynamic_cast<const IndexType*>(t1);
            const IndexType* it2 = dynamic_cast<const IndexType*>(t2);
        ...
        case TYPE_FUNC: {
            const FuncType* ft1 = dynamic_cast<const FuncType*>(t1);
            const FuncType* ft2 = dynamic_cast<const FuncType*>(t2);
        ...
        case TYPE_RECORD: {
            const RecordType* rt1 = dynamic_cast<const RecordType*>(t1);
            const RecordType* rt2 = dynamic_cast<const RecordType*>(t2);
...
TypePtr merge_func_types(const Type* t1, const Type* t2) {
    ...
    const FuncType* ft1 = dynamic_cast<const FuncType*>(t1);
    const FuncType* ft2 = dynamic_cast<const FuncType*>(t1);
