...
TableVal::TableVal(TableType* t, Attributes* a) : Val(t)
	...
	for ( const auto& t : *table_type->IndexTypes() )
		...
		for ( auto rt : found )
			parse_time_table_record_dependencies[rt].emplace_back(this->Ref()->AsTableVal());
		}
	}
...
void TableVal::SaveParseTimeTableState(RecordType* rt)
	...
	for ( auto& tv : table_vals )
		parse_time_table_states[tv] = tv->DumpTableState();
	}

void TableVal::RebuildParseTimeTables()
	{
	for ( auto& it : parse_time_table_states )
		{
		auto& tv = it.first;
		auto& ptts = it.second;
...
void TableVal::DoneParsing()
	{
	for ( auto& it : parse_time_table_record_dependencies )
		for ( auto& tv : it.second )
			Unref(tv);
...
TableVal::ParseTimeTableState TableVal::DumpTableState()
	...
	while ( (val = tbl->NextEntry(key, cookie)) )
		{
		rval.emplace_back(RecoverIndex(key), val->Value()->Ref());
		delete key;
		}
...
RecordVal::RecordVal(RecordType* t, bool init_fields) : Val(t)
	...
	if ( is_parsing )
		parse_time_records[t].emplace_back(this->Ref()->AsRecordVal());
...
void RecordVal::DoneParsing()
	{
	for ( auto& it : parse_time_records )
