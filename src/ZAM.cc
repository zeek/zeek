...
const char* abstract_op_name(ZOp op)
	{
	switch ( op ) {
...
static ZAM_tracker_type* curr_ZAM_VM_Tracker;
...
static ZAMVectorMgr* to_ZAM_vector(Val* vec, bool track_val);
static std::shared_ptr<ZAM_vector> to_raw_ZAM_vector(Val* vec);
...
static void grow_vector(ZAM_vector& vec, int new_size)
	{
...
// for consistency.
ZInst GenStmt(ZAM* m, ZOp op)
	{
	return ZInst(op);
	}

ZInst GenStmt(ZAM* m, ZOp op, const NameExpr* v1)
	{
...
ZInst GenStmt(ZAM* m, ZOp op, const ConstExpr* ce)
	{
	return ZInst(op, ce);
	}
ZInst GenStmt(ZAM* m, ZOp op, const NameExpr* v1,
...
ZInst GenStmt(ZAM* m, ZOp op, const NameExpr* v1,
	...
	{
	auto s = ZInst(op, m->FrameSlot(v1), m->FrameSlot(v2), i);
	s.op_type = OP_VVV_I3;
	return s;
	}


ZAM::ZAM(const BroFunc* f, Stmt* _body, UseDefs* _ud,
...
const CompiledStmt ZAM::InterpretExpr(const Expr* e)
	{
	FlushVars(e);
	return AddStmt(ZInst(OP_INTERPRET_EXPR_X, e));
	}

const CompiledStmt ZAM::InterpretExpr(const NameExpr* n,
							const Expr* e)
	{
	FlushVars(e);
	return AddStmt(ZInst(OP_INTERPRET_EXPR_V, FrameSlot(n), e));
	}

const CompiledStmt ZAM::DoCall(const CallExpr* c,
...
const CompiledStmt ZAM::ArithCoerce(const NameExpr* n,
						const Expr* e)
	{
	auto nt = n->Type();
	...
	return AddStmt(ZInst(a, n1, n2));
	}

const CompiledStmt ZAM::RecordCoerce(const NameExpr* n,
						const Expr* e)
	{
	...
	ZInst s(OP_RECORD_COERCE_VVV, FrameSlot(n), FrameSlot(op),
			map_size);
...
const CompiledStmt ZAM::TableCoerce(const NameExpr* n,
						const Expr* e)
	{
	auto op = e->GetOp1()->AsNameExpr();

	ZInst s(OP_TABLE_COERCE_VV, FrameSlot(n), FrameSlot(op));
	s.t = e->Type().get();

	return AddStmt(s);
	}

const CompiledStmt ZAM::VectorCoerce(const NameExpr* n,
						const Expr* e)
	{
	auto op = e->GetOp1()->AsNameExpr();
...
const CompiledStmt ZAM::IfElse(const NameExpr* n, const Stmt* s1,
						const Stmt* s2)
	{
	ZOp op = (s1 && s2) ?
...
const CompiledStmt ZAM::While(const Stmt* cond_stmt,
	...
	auto cond_IF = AddStmt(ZInst(OP_IF_VV, FrameSlot(cond), 0));
...
const CompiledStmt ZAM::ValueSwitch(const SwitchStmt* sw,
						const NameExpr* v,
						const ConstExpr* c)
	{
	...
	if ( c )
		...
		slot = RegisterSlot();
		auto s = ZInst(OP_ASSIGN_CONST_VC, slot, c);
		body_end = AddStmt(s);
		}
...
const CompiledStmt ZAM::TypeSwitch(const SwitchStmt* sw,
						const NameExpr* v,
						const ConstExpr* c)
	{
	auto cases = sw->Cases();
	...
	if ( v && v->Type()->Tag() != TYPE_ANY )
		{
		auto s = ZInst(OP_ASSIGN_ANY_VV, tmp, slot);
		body_end = AddStmt(s);
		slot = tmp;
		}
	...
	if ( c )
		{
		auto s = ZInst(OP_ASSIGN_ANY_VC, tmp, c);
		body_end = AddStmt(s);
		slot = tmp;
		}
	...
	for ( auto& i : *type_map )
		...
		s = ZInst(OP_BRANCH_IF_NOT_TYPE_VV, slot, 0);
		s.t = type;
		s.op_type = OP_VV_I2;
...
const CompiledStmt ZAM::AssignVecElems(const Expr* e)
	...
	if ( op2->Tag() == EXPR_NAME )
		...
		auto tmp = RegisterSlot();
		auto s = ZInst(OP_ASSIGN_VC, tmp, c3);
		s.t = c3->Type().get();
...
const CompiledStmt ZAM::LoopOverTable(const ForStmt* f,
							const NameExpr* val)
	{
	...
	auto info = NewSlot();
	auto s = ZInst(OP_INIT_TABLE_LOOP_VV, info, FrameSlot(val));
	...
	for ( int i = 0; i < loop_vars->length(); ++i )
		{
		auto id = (*loop_vars)[i];
		s = ZInst(OP_ADD_VAR_TO_INIT_VV, info, FrameSlot(id));
		s.t = id->Type();
		init_end = AddStmt(s);
		}
	...
	if ( value_var )
		{
		s = ZInst(OP_NEXT_TABLE_ITER_VAL_VAR_VVV, info,
					FrameSlot(value_var), 0);
		s.op_type = OP_VVV_I3;
		}
	else
		{
		s = ZInst(OP_NEXT_TABLE_ITER_VV, info, 0);
		s.op_type = OP_VV_I2;
		}
...
const CompiledStmt ZAM::LoopOverVector(const ForStmt* f,
							const NameExpr* val)
	{
	auto loop_vars = f->LoopVars();
...
const CompiledStmt ZAM::LoopOverString(const ForStmt* f,
							const NameExpr* val)
	{
	auto loop_vars = f->LoopVars();
	auto loop_var = (*loop_vars)[0];
	...
	auto info = NewSlot();
	auto s = ZInst(OP_INIT_STRING_LOOP_VV, info, FrameSlot(val));
	auto init_end = AddStmt(s);

	s = ZInst(OP_NEXT_STRING_ITER_VVV, info, FrameSlot(loop_var), 0);
	s.op_type = OP_VVV_I3;

	return FinishLoop(s, f->LoopBody(), info);
	}

const CompiledStmt ZAM::FinishLoop(ZInst iter_stmt,
						const Stmt* body,
						int info_slot)
	{
	...
	auto final_stmt = AddStmt(ZInst(OP_END_LOOP_V, info_slot));
...
const CompiledStmt ZAM::InitRecord(ID* id, RecordType* rt)
	{
	auto s = ZInst(OP_INIT_RECORD_V, FrameSlot(id));
	s.t = rt;
	return AddStmt(s);
	}
...
const CompiledStmt ZAM::InitTable(ID* id, TableType* tt,
						Attributes* attrs)
	{
	auto s = ZInst(OP_INIT_TABLE_V, FrameSlot(id));
	s.t = tt;
	s.attrs = attrs;
	return AddStmt(s);
	}
...
int ZAM::InternalBuildVals(const ListExpr* l)
	...
	auto s = ZInst(OP_CREATE_VAL_VEC_V, tmp, n);
	s.op_type = OP_VV_I2;
	(void) AddStmt(s);
...
const CompiledStmt ZAM::AddStmt(const ZInst& stmt)
	{
...
const CompiledStmt ZAM::LoadOrStoreLocal(ID* id, bool is_load,
							bool add)
	{
	if ( id->AsType() )
...
const CompiledStmt ZAM::LoadOrStoreGlobal(ID* id, bool is_load,
							bool add)
	{
	if ( id->AsType() )
...
const CompiledStmt ZAM::CompileInExpr(const NameExpr* n1,
	...
	auto s = ZInst(OP_TRANSFORM_VAL_VEC_TO_LIST_VAL_VVV,
	...
	s = ZInst(op, FrameSlot(n1), FrameSlot(n2), build_indices);

	return AddStmt(s);
	}

const CompiledStmt ZAM::CompileIndex(const NameExpr* n1,
							const NameExpr* n2,
							const ListExpr* l)
	{
	ZInst s;
	...
	if ( n == 1 && n2t == TYPE_STRING )
		...
		if ( ind->Tag() == EXPR_NAME )
			...
			s = ZInst(OP_INDEX_STRING_VVV, FrameSlot(n1),
			...
			s = ZInst(OP_INDEX_STRINGC_VVV, FrameSlot(n1),
					FrameSlot(n2), c);
			s.op_type = OP_VVV_I3;
			}
	...
	auto build_indices = InternalBuildVals(l);
	s = ZInst(OP_TRANSFORM_VAL_VEC_TO_LIST_VAL_VVV,
				build_indices, build_indices, n);
	s.op_type = OP_VVV_I3;
	AddStmt(s);
	...
	case TYPE_VECTOR:
		...
		s = ZInst(op, FrameSlot(n1), FrameSlot(n2),
					build_indices);
		s.t = n2->Type().get();
		break;
		}
	...
	case TYPE_TABLE:
		s = ZInst(OP_TABLE_INDEX_VVV, FrameSlot(n1),
					FrameSlot(n2), build_indices);
		s.t = n1->Type().get();
		break;

	case TYPE_STRING:
		s = ZInst(OP_INDEX_STRING_SLICE_VVL, FrameSlot(n1),
					FrameSlot(n2), build_indices);
		s.t = n1->Type().get();
		break;
...
const CompiledStmt ZAM::CompileSchedule(const NameExpr* n,
	...
	if ( n )
		s = ZInst(OP_SCHEDULE_ViHL, FrameSlot(n),
					is_interval, build_indices);
	else
		s = ZInst(OP_SCHEDULE_CiHL, is_interval,
					build_indices, c);
...
const CompiledStmt ZAM::CompileEvent(EventHandler* h,
							const ListExpr* l)
	{
	int len = l->Exprs().length();
	auto build_indices = InternalBuildVals(l);

	ZInst s(OP_EVENT_HL, build_indices);
	s.event_handler = h;
