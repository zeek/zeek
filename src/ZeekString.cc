...
String::Vec* String::Split(const String::IdxVec& indices) const {
    ...
    size_t i = 0;
    // This is a little unintuitive, but "first" here is the first element
    // of the non-unique elements from std::ranges::unique. That is, it's
    // the first element after the last unique element.
    for ( IdxVecIt it = idx.begin(); it != first; ++it, ++i ) {
