...
public:
	 ...
	 */
	[[deprecated("Remove in v4.1.  Use ConnVal() instead.")]] RecordVal* BuildConnVal();
	 ...
	 * Connection::Event().
	 */
	[[deprecated("Remove in v4.1.  Use EnqueueConnEvent() instead (note it doesn't automatically "
	             "ad the connection argument).")]] void
	Event(EventHandlerPtr f, Val* v1, Val* v2 = nullptr);

	/**
	 * Convenience function that forwards directly to
	 * Connection::ConnectionEvent().
	 */
	[[deprecated("Remove in v4.1.  Use EnqueueConnEvent() instead.")]] void
	ConnectionEvent(EventHandlerPtr f, ValPList* vl);
	 ...
	 */
	[[deprecated("Remove in v4.1.  Use EnqueueConnEvent() instead.")]] void
	ConnectionEventFast(EventHandlerPtr f, ValPList vl);
...
#define LOOP_OVER_CHILDREN(var)                                                                    \
	for ( zeek::analyzer::analyzer_list::iterator var = children.begin(); var != children.end();   \
	      var++ )
...
#define LOOP_OVER_GIVEN_CHILDREN(var, the_kids)                                                    \
	for ( zeek::analyzer::analyzer_list::iterator var = the_kids.begin(); var != the_kids.end();   \
	      var++ )
...
public:
	 ...
	 * @param conn The connection the analyzer is associated with.
	 */
	TransportLayerAnalyzer(const char* name, Connection* conn) : Analyzer(name, conn)
		{
		pia = nullptr;
		}
...
using Analyzer [[deprecated("Remove in v4.1. Use zeek::analyzer::Analyzer instead.")]] =
	zeek::analyzer::Analyzer;
using AnalyzerTimer [[deprecated("Remove in v4.1. Use zeek::analyzer::AnalyzerTimer instead.")]] =
	zeek::analyzer::AnalyzerTimer;
using SupportAnalyzer
	[[deprecated("Remove in v4.1. Use zeek::analyzer::SupportAnalyzer instead.")]] =
		zeek::analyzer::SupportAnalyzer;
using OutputHandler [[deprecated("Remove in v4.1. Use zeek::analyzer::OutputHandler instead.")]] =
	zeek::analyzer::OutputHandler;
using TransportLayerAnalyzer
	[[deprecated("Remove in v4.1. Use zeek::analyzer::TransportLayerAnalyzer instead.")]] =
		zeek::analyzer::TransportLayerAnalyzer;

using analyzer_list [[deprecated("Remove in v4.1. Use zeek::analyzer::analyzer_list instead.")]] =
	zeek::analyzer::analyzer_list;
