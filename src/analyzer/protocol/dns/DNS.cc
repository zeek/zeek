...
bool DNS_Interpreter::ParseRR_SOA(detail::DNS_MsgInfo* msg,
	...
	if ( dns_SOA_reply && ! msg->skip_event )
		...
		r->Assign(2, serial);
		r->Assign(3, double(refresh));
		r->Assign(4, double(retry));
		r->Assign(5, double(expire));
		r->Assign(6, double(minimum));
...
RecordValPtr DNS_MsgInfo::BuildRRSIG_Val(RRSIG_DATA* rrsig)
	...
	r->Assign(5, double(rrsig->orig_ttl));
	r->Assign(6, double(rrsig->sig_exp));
	r->Assign(7, double(rrsig->sig_incep));
