...
public:
    HTTP_Message(HTTP_Analyzer* analyzer, analyzer::tcp::ContentLine_Analyzer* cl, bool is_orig, int expect_body,
...
public:
    ...
    struct HTTP_VersionNumber {
        ...
        bool operator==(const HTTP_VersionNumber& other) const { return minor == other.minor && major == other.major; }
    ...
    static bool Available() {
        return (http_request || http_reply || http_header || http_all_headers || http_begin_entity || http_end_entity ||
                http_content_type || http_entity_data || http_message_done || http_event || http_stats);
    }
...
protected:
    ...
    const char* PrefixWordMatch(const char* line, const char* end_of_line, const char* prefix, bool ignore_case = false);
