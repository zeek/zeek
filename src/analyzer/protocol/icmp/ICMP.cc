...
ICMP_Analyzer::ICMP_Analyzer(Connection* c)
	: TransportLayerAnalyzer("ICMP", c), icmp_conn_val(), type(), code(), request_len(-1),
	  reply_len(-1)
	{
...
void ICMP_Analyzer::DeliverPacket(int len, const u_char* data, bool is_orig, uint64_t seq,
	...
	if ( ip->NextProto() == IPPROTO_ICMP )
		...
		{
		reporter->AnalyzerError(this, "expected ICMP as IP packet's protocol, got %d",
		                        ip->NextProto());
		return;
		}
...
void ICMP_Analyzer::ICMP_Sent(const struct icmp* icmpp, int len, int caplen, int icmpv6,
                              const u_char* data, const IP_Hdr* ip_hdr)
	{
	if ( icmp_sent )
		EnqueueConnEvent(icmp_sent, ConnVal(), BuildICMPVal(icmpp, len, icmpv6, ip_hdr),
	...
	if ( icmp_sent_payload )
		...
		EnqueueConnEvent(icmp_sent_payload, ConnVal(), BuildICMPVal(icmpp, len, icmpv6, ip_hdr),
		                 BuildInfo(icmpp, len, icmpv6, ip_hdr), make_intrusive<StringVal>(payload));
		}
	}

RecordValPtr ICMP_Analyzer::BuildICMPVal(const struct icmp* icmpp, int len, int icmpv6,
                                         const IP_Hdr* ip_hdr)
	{
	if ( ! icmp_conn_val )
		{
...
RecordValPtr ICMP_Analyzer::BuildInfo(const struct icmp* icmpp, int len, bool icmpv6,
                                      const IP_Hdr* ip_hdr)
	{
...
TransportProto ICMP_Analyzer::GetContextProtocol(const IP_Hdr* ip_hdr, uint32_t* src_port,
                                                 uint32_t* dst_port)
	{
...
unsigned int ICMP_Analyzer::MemoryAllocation() const
	{
	return Analyzer::MemoryAllocation() + padded_sizeof(*this) - padded_sizeof(Connection) +
	       (icmp_conn_val ? icmp_conn_val->MemoryAllocation() : 0);
	}

void ICMP_Analyzer::Echo(double t, const struct icmp* icmpp, int len, int caplen,
                         const u_char*& data, const IP_Hdr* ip_hdr)
	{
...
void ICMP_Analyzer::RouterAdvert(double t, const struct icmp* icmpp, int len, int caplen,
                                 const u_char*& data, const IP_Hdr* ip_hdr)
	{
	...
	EnqueueConnEvent(f, ConnVal(), BuildICMPVal(icmpp, len, 1, ip_hdr),
...
void ICMP_Analyzer::NeighborAdvert(double t, const struct icmp* icmpp, int len, int caplen,
                                   const u_char*& data, const IP_Hdr* ip_hdr)
	{
...
void ICMP_Analyzer::NeighborSolicit(double t, const struct icmp* icmpp, int len, int caplen,
                                    const u_char*& data, const IP_Hdr* ip_hdr)
	{
	...
	EnqueueConnEvent(f, ConnVal(), BuildICMPVal(icmpp, len, 1, ip_hdr),
	                 BuildInfo(icmpp, len, 1, ip_hdr), make_intrusive<AddrVal>(tgtaddr),
	                 BuildNDOptionsVal(caplen - opt_offset, data + opt_offset));
	}

void ICMP_Analyzer::Redirect(double t, const struct icmp* icmpp, int len, int caplen,
                             const u_char*& data, const IP_Hdr* ip_hdr)
	{
...
void ICMP_Analyzer::RouterSolicit(double t, const struct icmp* icmpp, int len, int caplen,
                                  const u_char*& data, const IP_Hdr* ip_hdr)
	{
	...
	EnqueueConnEvent(f, ConnVal(), BuildICMPVal(icmpp, len, 1, ip_hdr),
	                 BuildInfo(icmpp, len, 1, ip_hdr), BuildNDOptionsVal(caplen, data));
	}

void ICMP_Analyzer::Context4(double t, const struct icmp* icmpp, int len, int caplen,
                             const u_char*& data, const IP_Hdr* ip_hdr)
	{
	EventHandlerPtr f = nullptr;
	...
	if ( f )
		EnqueueConnEvent(f, ConnVal(), BuildICMPVal(icmpp, len, 0, ip_hdr),
		                 BuildInfo(icmpp, len, 0, ip_hdr), val_mgr->Count(icmpp->icmp_code),
		                 ExtractICMP4Context(caplen, data));
	}

void ICMP_Analyzer::Context6(double t, const struct icmp* icmpp, int len, int caplen,
                             const u_char*& data, const IP_Hdr* ip_hdr)
	{
	EventHandlerPtr f = nullptr;
	...
	if ( f )
		EnqueueConnEvent(f, ConnVal(), BuildICMPVal(icmpp, len, 1, ip_hdr),
		                 BuildInfo(icmpp, len, 1, ip_hdr), val_mgr->Count(icmpp->icmp_code),
...
VectorValPtr ICMP_Analyzer::BuildNDOptionsVal(int caplen, const u_char* data)
	...
	auto vv = make_intrusive<VectorVal>(id::find_type<VectorType>("icmp6_nd_options"));
