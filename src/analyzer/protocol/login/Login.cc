...
void Login_Analyzer::AuthenticationDialog(bool orig, char* line) {
    ...
    if ( prompt && ! IsSuccessMsg(line) && ! is_timeout ) {
        ...
        while ( (*prompt != '\0' && (next_prompt = IsLoginPrompt(prompt + 1))) || multi_line_prompt ) {
            if ( ! HaveTypeahead() ) {
        ...
        SetSkip(true);
              ...
              (login_prompt_line > 0 && lines_scanned > login_prompt_line + MAX_LOGIN_LOOKAHEAD + num_user_text) ) {
...
bool Login_Analyzer::IsPloy(const char* line) {
    if ( IsLoginPrompt(line) || IsFailureMsg(line) || IsSuccessMsg(line) || IsSkipAuthentication(line) ) {
        saw_ploy = true;
...
bool Login_Analyzer::IsFailureMsg(const char* line) const {
    return re_login_failure_msgs->MatchAnywhere(line) && ! re_login_non_failure_msgs->MatchAnywhere(line);
}

bool Login_Analyzer::IsSuccessMsg(const char* line) const { return re_login_success_msgs->MatchAnywhere(line); }

bool Login_Analyzer::IsTimeout(const char* line) const { return re_login_timeouts->MatchAnywhere(line); }
