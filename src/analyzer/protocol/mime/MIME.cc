...
bool is_null_data_chunk(data_chunk_t b) { return b.data == nullptr; }

bool is_lws(char ch) { return ch == 9 || ch == 32; }

StringValPtr to_string_val(int length, const char* data) { return make_intrusive<StringVal>(length, data); }

StringValPtr to_string_val(const char* data, const char* end_of_data) {
    return make_intrusive<StringVal>(end_of_data - data, data);
}

StringValPtr to_string_val(const data_chunk_t buf) { return to_string_val(buf.length, buf.data); }
...
void MIME_Mail::Undelivered(int len) {
    cur_entity_id =
        file_mgr->Gap(cur_entity_len, len, analyzer->GetAnalyzerTag(), analyzer->Conn(), is_orig, cur_entity_id);
}
...
bool MIME_is_field_name_char(char ch) { return ch >= 33 && ch <= 126 && ch != ':'; }
...
MIME_Header::~MIME_Header() { delete lines; }
...
void MIME_Entity::ParseMIMEHeader(MIME_Header* h) {
    ...
    switch ( current_field_type ) {
        case MIME_CONTENT_TYPE: ParseContentTypeField(h); break;

        case MIME_CONTENT_TRANSFER_ENCODING: ParseContentEncodingField(h); break;
    }
}
...
bool MIME_Entity::ParseContentTypeField(MIME_Header* h) {
    ...
    if ( content_type == CONTENT_TYPE_MULTIPART && ! multipart_boundary ) {
        IllegalFormat(
            "boundary delimiter is not specified for a multipart entity -- content is "
...
void MIME_Entity::ParseContentType(data_chunk_t type, data_chunk_t sub_type) {
    ...
    switch ( content_type ) {
        ...
        case CONTENT_TYPE_MESSAGE: need_to_parse_parameters = 1; break;

        default: need_to_parse_parameters = 0; break;
    }
}
...
void MIME_Entity::DecodeDataLine(int len, const char* data, bool trailing_CRLF) {
    ...
    switch ( content_encoding ) {
        case CONTENT_ENCODING_QUOTED_PRINTABLE: DecodeQuotedPrintable(len, data); break;

        case CONTENT_ENCODING_BASE64: DecodeBase64(len, data); break;

        case CONTENT_ENCODING_7BIT:
        case CONTENT_ENCODING_8BIT:
        case CONTENT_ENCODING_BINARY:
        case CONTENT_ENCODING_OTHER: DecodeBinary(len, data, trailing_CRLF); break;
    }
    FlushData();
}
...
void MIME_Entity::SubmitData(int len, const char* buf) { message->SubmitData(len, buf); }
...
void MIME_Entity::SubmitHeader(MIME_Header* h) { message->SubmitHeader(h); }

void MIME_Entity::SubmitAllHeaders() { message->SubmitAllHeaders(headers); }
...
MIME_Mail::MIME_Mail(analyzer::Analyzer* mail_analyzer, bool orig, int buf_size) : MIME_Message(mail_analyzer) {
    ...
    if ( mime_content_hash ) {
        ...
        sha1_hash = zeek::detail::hash_init(detail::Hash_SHA1);
    }
    else
...
void MIME_Mail::Done() {
    ...
    if ( compute_content_hash && mime_content_hash ) {
        u_char* digest = new u_char[ZEEK_SHA_DIGEST_LENGTH];
        zeek::detail::hash_final(sha1_hash, digest);
        sha1_hash = nullptr;

        analyzer->EnqueueConnEvent(mime_content_hash, analyzer->ConnVal(), val_mgr->Count(content_hash_length),
                                   make_intrusive<StringVal>(new String(true, digest, ZEEK_SHA_DIGEST_LENGTH)));
    }
...
MIME_Mail::~MIME_Mail() {
    if ( sha1_hash )
        zeek::detail::hash_state_free(sha1_hash);
...
void MIME_Mail::SubmitData(int len, const char* buf) {
    ...
    if ( compute_content_hash ) {
        ...
        zeek::detail::hash_update(sha1_hash, (const u_char*)buf, len);
    }
    ...
    cur_entity_id = file_mgr->DataIn(reinterpret_cast<const u_char*>(buf), len, analyzer->GetAnalyzerTag(),
...
void MIME_Mail::SubmitEvent(int event_type, const char* detail) {
    ...
    switch ( event_type ) {
        case MIME_EVENT_ILLEGAL_FORMAT: category = "illegal format"; break;

        case MIME_EVENT_ILLEGAL_ENCODING: category = "illegal encoding"; break;

        default: reporter->AnalyzerError(GetAnalyzer(), "unrecognized MIME_Mail event"); return;
    }
