...
void NetbiosSSN_Interpreter::ParseMessage(unsigned int type, unsigned int flags, const u_char* data,
                                          int len, bool is_query)
	{
	if ( netbios_session_message )
		analyzer->EnqueueConnEvent(netbios_session_message, analyzer->ConnVal(),
		                           val_mgr->Bool(is_query), val_mgr->Count(type),
		                           val_mgr->Count(len));
...
void NetbiosSSN_Interpreter::Event(EventHandlerPtr event, const u_char* data, int len, int is_orig)
	...
	if ( is_orig >= 0 )
		...
		analyzer->EnqueueConnEvent(event, analyzer->ConnVal(),
		                           make_intrusive<StringVal>(new String(data, len, false)));
	}
...
void NetbiosSSN_Analyzer::DeliverPacket(int len, const u_char* data, bool orig, uint64_t seq,
...
void NetbiosSSN_Analyzer::ExpireTimer(double t)
	...
	if ( run_state::terminating ||
	     run_state::network_time - Conn()->LastTime() >= netbios_ssn_session_timeout - 1.0 )
		{
