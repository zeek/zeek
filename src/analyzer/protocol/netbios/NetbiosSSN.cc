...
NetbiosDGM_RawMsgHdr::NetbiosDGM_RawMsgHdr(const u_char*& data, int& len) {
    ...
    len -= 2;
    MAKE_INT16(length, data);
    len -= 2;
    MAKE_INT16(offset, data);
    ;
    len -= 2;
}
...
void NetbiosSSN_Interpreter::ParseMessage(unsigned int type, unsigned int flags, const u_char* data, int len,
    ...
    if ( netbios_session_message )
        analyzer->EnqueueConnEvent(netbios_session_message, analyzer->ConnVal(), val_mgr->Bool(is_query),
                                   val_mgr->Count(type), val_mgr->Count(len));

    switch ( type ) {
        case NETBIOS_SSN_MSG: ParseSessionMsg(data, len, is_query); break;

        case NETBIOS_SSN_REQ: ParseSessionReq(data, len, is_query); break;

        case NETBIOS_SSN_POS_RESP: ParseSessionPosResp(data, len, is_query); break;

        case NETBIOS_SSN_NEG_RESP: ParseSessionNegResp(data, len, is_query); break;

        case NETBIOS_SSN_RETARG_RESP: ParseRetArgResp(data, len, is_query); break;

        case NETBIOS_SSN_KEEP_ALIVE: ParseKeepAlive(data, len, is_query); break;

        case NETBIOS_DGM_DIRECT_UNIQUE:
        case NETBIOS_DGM_DIRECT_GROUP:
        case NETBIOS_DGM_BROADCAST: ParseBroadcast(data, len, is_query); break;

        case NETBIOS_DGM_ERROR:
        case NETBIOS_DGG_QUERY_REQ:
        case NETBIOS_DGM_POS_RESP:
        case NETBIOS_DGM_NEG_RESP: ParseDatagram(data, len, is_query); break;

        default: analyzer->Weird("unknown_netbios_type", util::fmt("0x%x", type)); break;
    }
}
...
int NetbiosSSN_Interpreter::ConvertName(const u_char* name, int name_len, u_char*& xname, int& xlen) {
    ...
    *convert_name = 0;
    ...
    while ( len-- ) {
        if ( name[0] < 'A' || name[0] > 'P' || name[1] < 'A' || name[1] > 'P' ) {
            *convert_name = 0;
            return 0;
        }
...
void NetbiosSSN_Interpreter::ParseSessionReq(const u_char* data, int len, bool is_query) {
    if ( ! is_query )
...
void NetbiosSSN_Interpreter::ParseSessionPosResp(const u_char* data, int len, bool is_query) {
    if ( is_query )
...
void NetbiosSSN_Interpreter::ParseSessionNegResp(const u_char* data, int len, bool is_query) {
    if ( is_query )
        analyzer->Weird("netbios_client_session_reply");
...
void NetbiosSSN_Interpreter::ParseRetArgResp(const u_char* data, int len, bool is_query) {
    if ( is_query )
        analyzer->Weird("netbios_client_session_reply");
...
void NetbiosSSN_Interpreter::ParseKeepAlive(const u_char* data, int len, bool is_query) {
    Event(netbios_session_keepalive, data, len);
}

void NetbiosSSN_Interpreter::Event(EventHandlerPtr event, const u_char* data, int len, int is_orig) {
    if ( ! event )
        return;

    if ( is_orig >= 0 )
        analyzer->EnqueueConnEvent(event, analyzer->ConnVal(), val_mgr->Bool(is_orig),
...
void NetbiosSSN_Analyzer::ConnectionClosed(analyzer::tcp::TCP_Endpoint* endpoint, analyzer::tcp::TCP_Endpoint* peer,
                                           bool gen_event) {
