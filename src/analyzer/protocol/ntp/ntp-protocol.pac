type NTP_PDU(is_orig: bool) = record {
  ...
  mode_chk_1     : case (mode>=1 && mode<=5) of {
  	true  -> msg: NTP_Association(first_byte, mode);
    false -> unk: empty;
  };
  mode_chk_2     : case (mode) of {
    6 -> ctl_msg : bytestring &restofdata;
    7 -> mode_7  : NTP_Mode7(first_byte) &restofdata;
...
type NTP_Association(first_byte: uint8, mode: uint8) = record {
	stratum        : uint8;
	poll           : int8;
	precision      : int8;
...
#
type NTP_Mode7(first_byte: uint8) = record {
	second_byte       : uint8;
