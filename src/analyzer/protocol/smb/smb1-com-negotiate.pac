...
refine connection SMB_Conn += {
	...
	function proc_smb1_negotiate_request(header: SMB_Header, val: SMB1_negotiate_request): bool
		...
		if ( smb1_negotiate_request )
			{
			auto dialects = zeek::make_intrusive<VectorVal>(zeek::id::string_vec);
	...
	function proc_smb1_negotiate_response(header: SMB_Header, val: SMB1_negotiate_response): bool
		...
		if ( smb1_negotiate_response )
			{
			auto response = zeek::make_intrusive<RecordVal>(zeek::BifType::Record::SMB1::NegotiateResponse);

			switch ( ${val.word_count} )
				{
				case 0x01:
					{
					auto core = zeek::make_intrusive<RecordVal>(zeek::BifType::Record::SMB1::NegotiateResponseCore);
				...
				case 0x0d:
					{
					auto security = zeek::make_intrusive<RecordVal>(zeek::BifType::Record::SMB1::NegotiateResponseSecurity);
					...
					auto raw = zeek::make_intrusive<RecordVal>(zeek::BifType::Record::SMB1::NegotiateRawMode);
					raw->Assign(0, val_mgr->Bool(${val.lanman.raw_read_supported}));
					raw->Assign(1, val_mgr->Bool(${val.lanman.raw_write_supported}));

					auto lanman = zeek::make_intrusive<RecordVal>(zeek::BifType::Record::SMB1::NegotiateResponseLANMAN);
				...
				case 0x11:
					...
					auto capabilities = zeek::make_intrusive<RecordVal>(zeek::BifType::Record::SMB1::NegotiateCapabilities);
					...
					auto ntlm = zeek::make_intrusive<RecordVal>(zeek::BifType::Record::SMB1::NegotiateResponseNTLM);
