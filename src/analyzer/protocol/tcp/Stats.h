...
public:
	...
	void ChangeState(EndpointState o_prev, EndpointState o_now, EndpointState r_prev,
	                 EndpointState r_now);
	...
	void StateLeft(EndpointState o_state, EndpointState r_state) { --state_cnt[o_state][r_state]; }

	unsigned int Cnt(EndpointState state) const { return Cnt(state, state); }
	unsigned int Cnt(EndpointState state1, EndpointState state2) const
		{
		return state_cnt[state1][state2];
		}

	unsigned int NumStateEstablished() const { return Cnt(TCP_ENDPOINT_ESTABLISHED); }
	...
	unsigned int NumStateClosed() const { return Cnt(TCP_ENDPOINT_CLOSED); }
	...
	unsigned int NumStateInactive() const { return Cnt(TCP_ENDPOINT_INACTIVE); }
...
using TCPStateStats [[deprecated("Remove in v4.1. Use zeek::analyzer::tcp::TCPStateStats.")]] =
	zeek::analyzer::tcp::TCPStateStats;
