...
void TCP_Endpoint::SetState(EndpointState new_state)
	...
	if ( new_state != state )
		...
		state = new_state;
		if ( IsOrig() )
			sessions->tcp_stats.ChangeState(prev_state, state, peer->state, peer->state);
		else
			sessions->tcp_stats.ChangeState(peer->state, peer->state, prev_state, state);
		}
	}
	...
	if ( contents_file && ! contents_processor && seq + len > contents_start_seq )
		...
		if ( fwrite(data, 1, len, f) < unsigned(len) )
			...
			if ( contents_file_write_failure )
				tcp_analyzer->EnqueueConnEvent(contents_file_write_failure, Conn()->ConnVal(),
				                               val_mgr->Bool(IsOrig()),
