...
bool ZeroMQBackend::DoPublishEvent(const std::string& topic, const std::string& format, const util::byte_buffer& buf) {
...
void ZeroMQBackend::Run() {
    ...
    auto HandleLogMessages = [this](const std::vector<MultipartMessage>& msgs) {
        for ( const auto& msg : msgs ) {
            ...
            util::byte_buffer payload{msg[3].data<std::byte>(), msg[3].data<std::byte>() + msg[3].size()};
    ...
    auto HandleXPubMessages = [this](const std::vector<MultipartMessage>& msgs) {
        for ( const auto& msg : msgs ) {
            ...
            if ( first == 0 || first == 1 ) {
                ...
                util::byte_buffer topic(start, end);
                if ( first == 1 ) {
...
bool ZeroMQBackend::DoProcessBackendMessage(int tag, util::byte_buffer_span payload) {
    if ( tag == 0 || tag == 1 ) {
