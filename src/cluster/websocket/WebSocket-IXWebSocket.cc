...
namespace zeek::cluster::websocket::ixwebsocket {
...
std::unique_ptr<WebSocketServer> StartServer(std::unique_ptr<WebSocketEventDispatcher> dispatcher,
    ...
    auto server =
        std::make_unique<ix::WebSocketServer>(options.port, options.host, ix::SocketServer::kDefaultTcpBacklog,
                                              ...
                                              ix::SocketServer::kDefaultAddressFamily, options.max_connections);

    if ( ! options.per_message_deflate )
        server->disablePerMessageDeflate();

    if ( tls_options.enable ) {
        ix::SocketTLSOptions ix_tls_options{};
        ix_tls_options.tls = true;
        ix_tls_options.certFile = tls_options.cert_file;
        ix_tls_options.keyFile = tls_options.key_file;
    ...
    ix::WebSocketServer::OnConnectionCallback cb =
        ...
        ix::OnMessageCallback cb = [dispatcher, id, remotePort, remoteIp,
            ...
            if ( msg->type == ix::WebSocketMessageType::Open ) {
                ...
                fprintf(stderr, "[ERROR] websocket error of %s (%s:%d): %s", id.c_str(), remoteIp.c_str(), remotePort,
                        msg->errorInfo.reason.c_str());
                dispatcher->QueueForProcessing(WebSocketClose{id});
            }
        };

        ws->setOnMessageCallback(cb);
    };

    server->setOnConnectionCallback(cb);
...
std::unique_ptr<WebSocketServer> zeek::cluster::websocket::StartServer(
    std::unique_ptr<WebSocketEventDispatcher> dispatcher, const ServerOptions& options, const TLSOptions& tls_options) {
    // Just delegate to the above IXWebSocket specific implementation.
    return ixwebsocket::StartServer(std::move(dispatcher), options, tls_options);
}
