...
using namespace zeek::cluster::websocket;
...
void WebSocketEventDispatcher::Process(WebSocketEvent&& event) {
    std::visit([this](auto&& arg) { Process(arg); }, event);
}

void WebSocketEventDispatcher::Process(WebSocketOpen& open) {
...
void WebSocketEventDispatcher::Process(WebSocketClose& close) {
...
void WebSocketEventDispatcher::Process(WebSocketSubscribeFinished& fin) {
...
void WebSocketEventDispatcher::HandleSubscriptions(WebSocketClientEntry& entry, std::string buf) {
...
void WebSocketEventDispatcher::HandleEvent(WebSocketClientEntry& entry, std::string buf) {
    // Unserialize the message as an event.
    broker::variant res;
    std::string topic;
    auto err = broker::format::json::v1::decode(buf, res, &topic);
    if ( err ) {
    ...
    const auto& zeek_ev = detail::to_zeek_event(broker_ev);
    if ( ! zeek_ev ) {
...
void WebSocketEventDispatcher::Process(WebSocketMessage& msg) {
    ...
    WS_DEBUG("Message %zu size=%zu from %s (%s:%d) backend=%p", entry.msg_count, msg.msg.size(), wsc->getId().c_str(),
             wsc->getRemoteIp().c_str(), wsc->getRemotePort(), entry.backend.get());
    ...
    if ( entry.msg_count == 1 ) {
        ...
        HandleEvent(entry, std::move(msg.msg));
    }
}
