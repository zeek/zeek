...
public:
     ...
     */
    zeek::expected<zeek::ConnKeyPtr, std::string> ConnKeyFromVal(const zeek::ValPtr& v) const {
...
protected:
     ...
     * @return A unique pointer to the ConnKey instance, or an error message.
     */
    virtual zeek::expected<zeek::ConnKeyPtr, std::string> DoConnKeyFromVal(const zeek::ValPtr& v) const = 0;
};
