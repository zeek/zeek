...
RecordValPtr X509::ParseCertificate(X509Val* cert_val, file_analysis::File* f)
	...
	pX509Cert->Assign(
		5, make_intrusive<TimeVal>(GetTimeFromAsn1(X509_get_notBefore(ssl_cert), f, reporter)));
	pX509Cert->Assign(
		6, make_intrusive<TimeVal>(GetTimeFromAsn1(X509_get_notAfter(ssl_cert), f, reporter)));
...
void X509::ParseBasicConstraints(X509_EXTENSION* ex)
	...
	if ( constr )
		...
		if ( x509_ext_basic_constraints )
			...
			if ( constr->pathlen )
				pBasicConstraint->Assign(
					1, val_mgr->Count((int32_t)ASN1_INTEGER_get(constr->pathlen)));
