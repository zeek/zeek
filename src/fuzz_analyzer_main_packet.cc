...
int _zeek_init(int argc, char** argv)
	{
	ZEEK_LSAN_DISABLE();
...
#ifndef FUZZ_ANALYZER
#define FUZZ_ANALYZER "pop3"
#endif
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
	char *argv[2] = {"bro", "local"};
	int argc = 2;
	static int initialized=_zeek_init(argc, argv);
	if(0 != buffer_splitter_validate(data, size))
		return 0;

	sessions = new NetSessions(); // normally done in net_init
	pkt_timeval ts = {0, 0};

	buffer_splitter *pp = buffer_splitter_init(data, size);
	if(pp == NULL)
		return 0;
	int err;
	unsigned char *chunk;
	...
	while(true) {
		...
		if (chunk_size==0)
			break;
		//fprintf(stderr, "Got chunk of length=%d, is_orig=%d\n", chunk_size, is_orig);
		Packet pkt;
		//Copy packet data so a read overflow gets flagged
		pkt.Init(DLT_RAW, &ts, chunk_size, chunk_size, chunk, true, "");
