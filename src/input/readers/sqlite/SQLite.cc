...
Value* SQLite::EntryToVal(sqlite3_stmt* st, const threading::Field* field, int pos, int subpos) {
    ...
    switch ( field->type ) {
        ...
        case TYPE_INT: val->val.int_val = sqlite3_column_int64(st, pos); break;

        case TYPE_DOUBLE:
        case TYPE_TIME:
        case TYPE_INTERVAL: val->val.double_val = sqlite3_column_double(st, pos); break;

        case TYPE_COUNT: val->val.uint_val = sqlite3_column_int64(st, pos); break;
    ...
    for ( int i = 0; i < numcolumns; ++i ) {
        ...
        for ( unsigned j = 0; j < num_fields; j++ ) {
            if ( strcmp(fields[j]->name, name) == 0 ) {
                if ( mapping[j] != -1 ) {
                    Error(
                        Fmt("SQLite statement returns several columns with name %s! Cannot "
