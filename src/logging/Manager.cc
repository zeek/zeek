...
bool Manager::Write(EnumVal* id, RecordVal* columns_arg)
	{
	// In-progress. :)
	// TODO: Add an option to create a span event with the log record, maybe instead of
	// (or in addition to) creating the span here and downstream spans. That way,
	// someone would have the option to (1) keep span counts low if they just want to see
	// what gets logged (and/)or (2) see the detailed processing through the logging
	// frontend and backend threads.

	// We add the span event for the log to the parent span, since that's where it will be most
	// valuable
	auto parent_span = zeek::trace::tracer->GetCurrentSpan();
	if ( parent_span->IsRecording() )
		parent_span->AddEvent("log", {{"columns", columns_arg->ToJSON()->ToStdString()}});

	// We use the global tracer here because the later call to RecordToFilterVals
	// stores the trace context for the current span in the log record, and we want
	// to be sure that's a recordable span.
	auto span = zeek::trace::tracer->StartSpan("Manager::Write");
	...
	for ( list<Filter*>::iterator i = stream->filters.begin(); i != stream->filters.end(); ++i )
		...
		/*
		// XXX NULL -> very bad
		auto formatter = new threading::formatter::JSON(NULL, threading::formatter::JSON::TS_EPOCH);
		auto odesc = ODesc();
		formatter->Describe(&odesc, filter->num_fields, filter->fields, vals[filter->num_fields]);
		*/
...
threading::Value** Manager::RecordToFilterVals(Stream* stream, Filter* filter, RecordVal* columns)
	...
	// We allocate an entry Value entry at the end for storing the otel span context
	threading::Value** vals = new threading::Value*[filter->num_fields + 1];
	...
	// For this to work, we need to make sure our current span is a recording span.
	// So, our span needs to be generated by the global tracer that is always enabled.
	auto current_ctx = opentelemetry::context::RuntimeContext::GetCurrent();
	auto span_context = opentelemetry::trace::GetSpan(current_ctx)->GetContext();
	auto trace_parent = zeek::trace::TraceParent(span_context);

	// XXX Is this safe?
	auto val = new threading::Value(zeek::TYPE_STRING, false);
	val->val.string_val.length = trace_parent.size();
	val->val.string_val.data = util::copy_string(trace_parent.c_str());

	// Store the otel span context after the log record's Values. Note that this is *past* the
	// last column Value tracked by num_fields, so pretty much all the logging code won't even
	// know this is here, with four exceptions:
	// 1. This method, Manager::RecordToFilterVals.
	// 2. WriterBackend::Write (which extracts and uses the span context from the log record).
	// 3. Manager::DeleteVals
	// 4. WriteBackend::DeleteVals (it's not clear why this is duplicated)
	vals[filter->num_fields] = val;
...
void Manager::DeleteVals(int num_fields, threading::Value** vals)
	...
	// We need to delete num_fields and + 1 for the span context. See the bottom of
	// RecordToFilterVals.
	for ( int i = 0; i <= num_fields; i++ )
		delete vals[i];
...
bool Manager::WriteFromRemote(EnumVal* id, EnumVal* writer, const string& path, int num_fields,
	...
	{
	auto span = zeek::trace::tracer->StartSpan("Manager::WriteFromRemote");
	auto scope = zeek::trace::tracer->WithActiveSpan(span);
