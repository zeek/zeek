...
void Manager::InitPostScript() {
    ...
    max_field_string_bytes = id::find_val("Log::limit_field_string_bytes")->AsCount();
    if ( max_field_string_bytes == 0 )
        max_field_string_bytes = std::numeric_limits<size_t>::max();

    max_total_string_bytes = id::find_val("Log::limit_total_string_bytes")->AsCount();
    if ( max_total_string_bytes == 0 )
        max_total_string_bytes = std::numeric_limits<size_t>::max();

    max_field_container_elements = id::find_val("Log::limit_field_container_elements")->AsCount();
    if ( max_field_container_elements == 0 )
        max_field_container_elements = std::numeric_limits<size_t>::max();

    max_total_container_elements = id::find_val("Log::limit_total_container_elements")->AsCount();
...
threading::Value Manager::ValToLogVal(std::optional<ZVal>& val, Type* ty, size_t& total_size) {
    ...
    switch ( lval.type ) {
        ...
        case TYPE_STRING: {
            ...
            if ( std::cmp_greater(s->Len(), allowed_bytes) )
                fprintf(stderr, "filtered string field %d %zu %zu %zu\n", s->Len(), max_field_string_bytes,
                        max_total_string_bytes, total_string_bytes);

            fprintf(stderr, "allowed bytes for string field: %zu\n", allowed_bytes);
        ...
        case TYPE_TABLE: {
            ...
            if ( std::cmp_greater(set->Length(), allowed_elements) )
                fprintf(stderr, "filtered st %d %zu %zu %zu\n", set->Length(), max_field_container_elements,
                        max_total_container_elements, total_container_elements);

            fprintf(stderr, "allowed elements for set: %zu\n", allowed_elements);
        ...
        case TYPE_VECTOR: {
            ...
            if ( std::cmp_greater(vec->Size(), allowed_elements) )
                fprintf(stderr, "filtered vector %d %zu %zu %zu\n", vec->Size(), max_field_container_elements,
                        max_total_container_elements, total_container_elements);

            fprintf(stderr, "allowed elements for vector: %zu\n", allowed_elements);
