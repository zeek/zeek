...
bool Manager::WritesFromRemote(const detail::LogWriteHeader& header, std::vector<detail::LogRecord>&& records) {
    ...
    // frontends buffering to be in effect.
    //
    // This is nice as it'll buffer log writes from different
    // remote nodes, but could also see how flushing the buffer
    // after every remote write might be more predictable.
    for ( auto& r : records )
