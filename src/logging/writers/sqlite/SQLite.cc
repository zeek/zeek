...
bool SQLite::DoInit(const WriterInfo& info, int arg_num_fields, const Field* const* arg_fields) {
    ...
    switch ( synchronous ) {
        ...
        case SQLITE_SYNCHRONOUS_EXTRA:
            ...
            break;
        default: Error(Fmt("Invalid LogSQLite::synchronous enum: %ld", synchronous)); return false;
    }
    if ( res != SQLITE_OK ) {
    ...
    switch ( journal_mode ) {
        ...
        default: Error(Fmt("Invalid LogSQLite::journal_mode enum: %ld", synchronous)); return false;
    }
    if ( res != SQLITE_OK ) {
