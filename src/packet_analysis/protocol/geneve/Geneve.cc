...
void zeek::packet_analysis::Geneve::detail::parse_options(zeek::Span<const uint8_t> data, detail::Callback cb) {
    ...
    const uint8_t* const end = &data[8] + opt_len;
    ...
    while ( p < end ) {
        ...
        uint16_t class_ = ntohs(reinterpret_cast<const uint16_t*>(p)[0]);
        bool critical = (p[2] & 0x80) == 0x80;
        uint8_t type = p[2] & 0x7F;
        uint8_t len = (p[3] & 0x1F) * 4;
        ...
        cb(class_, critical, type, zeek::Span{p, len});
