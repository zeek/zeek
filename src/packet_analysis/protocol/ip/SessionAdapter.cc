...
void EndpointRecordValCallback::AssignCallbacks(RecordVal* conn_val, const SessionAdapter* arg_adapter) {
    adapter = arg_adapter;

    orig_endp = conn_val->GetField<zeek::RecordVal>(orig_endp_offset).get();
    orig_endp->AssignCallback(size_offset, this);
    orig_endp->AssignCallback(state_offset, this);

    resp_endp = conn_val->GetField<zeek::RecordVal>(resp_endp_offset).get();
    resp_endp->AssignCallback(size_offset, this);
    resp_endp->AssignCallback(state_offset, this);
}

void EndpointRecordValCallback::RemoveCallbacks() {
...
void SessionAdapter::Done() {
    ...
    if ( endp_cb.HasCallbacksAssigned() )
        endp_cb.RemoveCallbacks();
}
...
void SessionAdapter::UpdateConnVal(RecordVal* conn_val) {
    if ( ! endp_cb.HasCallbacksAssigned() )
        endp_cb.AssignCallbacks(conn_val, this);
