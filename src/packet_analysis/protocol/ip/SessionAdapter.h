...
/**
 * Helper class embedded in the SessionAdapter class that allows
 * assign callbacks for size and state fields within the endpoint records
 * of connections.
 *
 * Call AssignCallbacks() the first time UpdateConnVal() is invoked on
 * a SessionAdapter. Use RemoveCallbacks() during Done().
 *
...
public:
    ...
    void AssignCallbacks(RecordVal* conn_val, const SessionAdapter* arg_adapter);

    void RemoveCallbacks();

    bool HasCallbacksAssigned() const noexcept { return adapter != nullptr; }
...
private:
    ...
    // Pointers to the endpoint records.
