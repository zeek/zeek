...
const struct tcphdr* TCPSessionAdapter::ExtractTCP_Header(const u_char*& data,
                                                          int& len, int& caplen)
	{
...
bool TCPSessionAdapter::ValidateChecksum(const IP_Hdr* ip, const struct tcphdr* tp,
...
void TCPSessionAdapter::SynWeirds(analyzer::tcp::TCP_Flags flags, analyzer::tcp::TCP_Endpoint* endpoint, int data_len) const
	{
	if ( flags.RST() )
