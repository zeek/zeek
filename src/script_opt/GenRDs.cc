...
RD_Decorate::RD_Decorate(std::shared_ptr<ProfileFunc> _pf, const Func* f, ScopePtr scope,
                         StmtPtr body)
	: pf(std::move(_pf))
	{
	...
	switch ( s->Tag() )
		...
		case STMT_CATCH_RETURN:
				...
				if ( ret_var )
					...
					// inlining, too.
					CreateInitPostDef(ret_var->Id(), DefinitionPoint(s), true, nullptr);
					}
...
void RD_Decorate::DoLoopConfluence(const Stmt* s, const Stmt* top, const Stmt* body)
	{
...
void RD_Decorate::AddBlockDefs(const Stmt* s, bool is_pre, bool is_future, bool is_case)
	{
...
void RD_Decorate::CheckVar(const Expr* e, const ID* id, bool check_fields)
	...
	if ( analysis_options.usage_issues > 0 && ! mgr.HasPreMinRD(e, id) &&
...
TraversalCode RD_Decorate::PreExpr(const Expr* e)
	...
	switch ( e->Tag() )
		...
		case EXPR_ASSIGN:
				...
				if ( lhs->Tag() == EXPR_LIST && rhs->GetType()->Tag() != TYPE_ANY )
					{
		...
		case EXPR_HAS_FIELD:
				...
				if ( r->Tag() == EXPR_NAME )
					...
					CreateInitPostDef(field_rd, DefinitionPoint(hf), true, 0);
		...
		case EXPR_CALL:
				...
				for ( const auto& expr : args_l->Exprs() )
					...
					if ( IsAggr(expr) )
						...
						mgr.CreatePostDef(expr->AsNameExpr()->Id(), DefinitionPoint(c), false);
					else
						expr->Traverse(this);
					}
...
void RD_Decorate::CreateInitPostDef(const ID* id, DefinitionPoint dp, bool assume_full,
                                    const Expr* rhs)
	{
	auto di = mgr.GetID_DI(id);
	if ( ! di )
...
void RD_Decorate::CreateInitPostDef(std::shared_ptr<DefinitionItem> di, DefinitionPoint dp,
                                    bool assume_full, const Expr* rhs)
	{
	CreateInitDef(std::move(di), dp, false, assume_full, rhs);
	}

void RD_Decorate::CreateInitDef(std::shared_ptr<DefinitionItem> di, DefinitionPoint dp, bool is_pre,
                                bool assume_full, const Expr* rhs)
	{
	if ( is_pre )
...
void RD_Decorate::CreateRecordRDs(std::shared_ptr<DefinitionItem> di, DefinitionPoint dp,
                                  bool is_pre, bool assume_full, const DefinitionItem* rhs_di)
	{
	...
	for ( auto i = 0; i < n; ++i )
		...
		else if ( ! rt->FieldHasAttr(i, ATTR_OPTIONAL) && ! is_atomic_type(t_i) )
		...
		if ( analysis_options.usage_issues > 1 )
			if ( t_i->Tag() == TYPE_RECORD )
				CreateRecordRDs(di_i, dp, is_pre, assume_full, rhs_di_i.get());
		}
	}

void RD_Decorate::CheckRecordRDs(std::shared_ptr<DefinitionItem> di, DefinitionPoint dp,
	...
	for ( auto i = 0; i < num_fields; ++i )
		{
		if ( rt->FieldHasAttr(i, ATTR_DEFAULT) || rt->FieldHasAttr(i, ATTR_OPTIONAL) ||
