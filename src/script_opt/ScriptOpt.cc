...
void analyze_scripts()
	...
	// Functions that are also used indirectly, and thus should be
	// compiled even if they were inlined.  We don't bother populating
	// this if we're not inlining, since it won't be consulted in
	// that case.
	...
	for ( auto& f : funcs )
		...
		optimize_func(func, f.Profile(), f.Scope(), new_body,
		              analysis_options);
		f.SetBody(new_body);
		}
