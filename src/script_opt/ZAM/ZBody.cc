...
ZBody::ZBody(std::string _func_name, const ZAMCompiler* zc) : Stmt(STMT_ZAM) {
    ...
    if ( analysis_options.profile_file ) {
        profile_calls = sample_CPU_mem = true;
        prof_sampling_rate = analysis_options.profile_sampling_rate;
    }
}
...
void ZBody::SetInsts(vector<ZInstI*>& instsI) {
    ...
    // Set up for tracking instruction counts.
...
ValPtr ZBody::Exec(Frame* f, StmtFlowType& flow) {
    ...
    // These are initialized if CPU/memory profiling is active, and only
    // accessed later if that's the case, too. We zero-initialize here because
    // -Werror=maybe-uninitialized gets confused.
    double start_CPU_time = 0.0;
    uint64_t start_mem = 0;

    bool do_CPU_mem_profile = false;
    // Similarly, these are only active if do_CPU_mem_profile is true.
    int profile_pc = 0;
    ...
    while ( pc < end_pc && ! ZAM_error ) {
        ...
        if ( sample_CPU_mem ) {
            ...
            do_CPU_mem_profile = seed % prof_sampling_rate == 0;
...
#include "ZAM-EvalDefs.h"
    ...
    if ( profile_calls ) {
        ...
        // We can detect that situation since when we adjust profile_calls,
        // we record during/after which call we did so. If that's the same
        // as the current call, then we should skip this measurement.
        if ( profiling_set_call < ncall ) {
...
uint64_t ZBody::NumBodyInsts() const {
    int num_inst = 0;
...
uint64_t ZBody::NumModuleInsts(const std::string& mod) const {
    int num_inst = 0;

    for ( unsigned i = 0; i < end_pc; ++i )
        if ( insts[i].ZAMLoc()->GetModules().contains(mod) )
            num_inst += inst_cnt[i];
...
void ZBody::ReportExecutionProfile(ProfMap& pm) {
    ...
    int total_samples = ncall + num_sampled_inst;
    double adj_CPU_time = tot_CPU_time;
    adj_CPU_time -= ncall * (mem_prof_overhead + CPU_prof_overhead);
    adj_CPU_time -= num_sampled_inst * CPU_prof_overhead;
    adj_CPU_time = std::max(adj_CPU_time, 0.0);

    fprintf(analysis_options.profile_file,
            "%s CPU time %.06f, %" PRIu64 " memory, %" PRIu64 " calls, %" PRIu64 " sampled instructions\n",
            func_name.c_str(), adj_CPU_time, tot_mem, ncall, num_sampled_inst);
