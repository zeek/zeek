...
OperationResult Redis::DoOpen(OpenResultCallback* cb, RecordValPtr options) {
    ...
    auto username_field = backend_options->GetField<StringVal>("acl_username");
    if ( username_field )
    ...
    if ( status == REDIS_OK ) {
        ...
        if ( ! password.empty() ) {
            if ( username.empty() )
                ...
                status =
                    redisAsyncCommand(async_ctx, redisAUTH, NULL, "AUTH %s %S", username.c_str(), password.c_str());
            ...
            if ( status == REDIS_ERR ) {
                ...
                CompleteCallback(open_cb, {ReturnCode::OPERATION_FAILED,
                                           util::fmt("AUTH command failed to authenticate: %s", async_ctx->errstr)});
            }
