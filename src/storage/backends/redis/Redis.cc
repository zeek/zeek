...
void redisPut(redisAsyncContext* ctx, void* reply, void* privdata) {
    // TODO: handle null reply here on error
    auto t = Tracer("put");
    auto backend = static_cast<zeek::storage::backends::redis::Redis*>(ctx->data);
...
void redisAddRead(void* privdata) {
    ...
    auto backend = static_cast<zeek::storage::backends::redis::Redis*>(privdata);
    backend->OnAddRead();
}
...
void Redis::Expire() {
    ...
    // TODO: it's possible to pass multiple keys to the DEL operation, but it
    // requires converting the replies below to a va_list. It might be faster to
    // do that than multiple commands though.
...
ValResult Redis::ParseGetReply(redisReply* reply) const {
    if ( ! reply )
        return nonstd::unexpected<std::string>("GET returned null reply");
    if ( ! reply->str )
        return nonstd::unexpected<std::string>("GET returned key didn't exist");
