...
process_stats get_process_stats() {
    ...
    // Fetch CPU time.
    {
        ...
        if ( task_info(mach_task_self(), TASK_THREAD_TIMES_INFO, reinterpret_cast<task_info_t>(&info), &count) ==
            ...
            result.user_cpu += info.user_time.seconds;
            result.user_cpu += ceil(info.user_time.microseconds / 1000.0) / 1000.0;
            result.system_cpu += info.system_time.seconds;
            result.system_cpu += ceil(info.system_time.microseconds / 1000.0) / 1000.0;
        }
    }
...
process_stats get_process_stats() {
    ...
    if ( auto f = fopen("/proc/self/stat", "r") ) {
        ...
        result.user_cpu = static_cast<double>(utime_ticks) / ticks_per_second;
        result.system_cpu = static_cast<double>(stime_ticks) / ticks_per_second;
...
process_stats get_process_stats() {
    ...
    if ( kp ) {
        ...
        result.user_cpu = static_cast<double>(kp->ki_rusage.ru_utime.tv_sec) +
                          (static_cast<double>(kp->ki_rusage.ru_utime.tv_nsec) / 1e6) result.system_cpu =
                              static_cast<double>(kp->ki_rusage.ru_stime.tv_sec) +
                              (static_cast<double>(kp->ki_rusage.ru_stime.tv_nsec) / 1e6)
