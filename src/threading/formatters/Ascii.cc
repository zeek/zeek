...
Ascii::Ascii(MsgThread* t, const SeparatorInfo& info) : Formatter(t) { separators = info; }
...
bool Ascii::Describe(ODesc* desc, Value* val, const string& name) const {
    ...
    switch ( val->type ) {
        case TYPE_BOOL: desc->Add(val->val.int_val ? "T" : "F"); break;

        case TYPE_INT: desc->Add(val->val.int_val); break;

        case TYPE_COUNT: desc->Add(val->val.uint_val); break;

        case TYPE_PORT: desc->Add(val->val.port_val.port); break;

        case TYPE_SUBNET: desc->Add(Render(val->val.subnet_val)); break;
    ...
    switch ( type ) {
        ...
        case TYPE_BOOL: {
            ...
            if ( stripped == "T" || stripped == "1" )
                ...
                GetThread()->Warning(GetThread()->Fmt("Field: %s Invalid value for boolean: %s", name.c_str(), start));
                goto parse_error;
            }
            break;
        }
        ...
        case TYPE_PORT: {
            ...
            if ( pos != std::string::npos && stripped.length() > pos + 1 ) {
                ...
                if ( util::strtolower(proto) == "tcp" )
                    ...
                    GetThread()->Warning(
                        GetThread()->Fmt("Port '%s' contained unknown protocol '%s'", s.c_str(), proto.c_str()));
            }
        ...
        case TYPE_VECTOR:
            ...
            // Then - common stuff
            {
                ...
                while ( splitstream ) {
                    ...
                    if ( pos >= length ) {
                        GetThread()->Warning(
                            GetThread()->Fmt("Internal error while parsing set. pos %d >= length %d."
                                             " Element: %s",
                ...
                if ( pos != length ) {
                    GetThread()->Warning(
                        GetThread()->Fmt("Internal error while parsing set: did not find all elements: %s", start));
                    goto parse_error;
                }
        ...
        default:
            GetThread()->Warning(GetThread()->Fmt("unsupported field format %d for %s", type, name.c_str()));
            goto parse_error;
    }
...
bool Ascii::CheckNumberError(const char* start, const char* end, bool nonneg_only) const {
    ...
    if ( (*end != '\0') )
        thread->Warning(
            thread->Fmt("Number '%s' contained non-numeric trailing characters. "
