...
static void WarnMissingScript(const char* type, const zeek::detail::ID* id, const string& script) {
    ...
    DbgAndWarn(
        util::fmt("Can't generate Zeekygen documentation for %s %s, "
                  "lookup of %s failed",
...
void Manager::ScriptDependency(const string& path, const string& dep) {
    ...
    if ( ! script_info ) {
        DbgAndWarn(
            util::fmt("Failed to add Zeekygen script doc dependency %s "
                      "for %s",
...
void Manager::ModuleUsage(const string& path, const string& module) {
    ...
    if ( ! script_info ) {
        DbgAndWarn(util::fmt("Failed to add Zeekygen module usage %s in %s", module.c_str(), name.c_str()));
        return;
    }
...
void Manager::StartType(zeek::detail::IDPtr id) {
    ...
    if ( *id->GetLocationInfo() == zeek::detail::no_location ) {
        DbgAndWarn(
            util::fmt("Can't generate zeekygen documentation for %s, "
    ...
    DBG_LOG(DBG_ZEEKYGEN, "Making IdentifierInfo (incomplete) %s, in %s", id->Name(), script.c_str());
    incomplete_type = CreateIdentifierInfo(std::move(id), script_info);
}

static bool IsEnumType(zeek::detail::ID* id) { return id->IsType() ? id->GetType()->Tag() == TYPE_ENUM : false; }
...
void Manager::RecordField(const zeek::detail::ID* id, const TypeDecl* field, const string& path, bool from_redef) {
    if ( disabled )
        return;
    ...
    if ( ! idd ) {
        DbgAndWarn(
            util::fmt("Can't generate zeekygen documentation for "
    ...
    comment_buffer.clear();
    DBG_LOG(DBG_ZEEKYGEN, "Document record field %s, identifier %s, script %s", field->id, id->Name(), script.c_str());
}
...
void Manager::SummaryComment(const string& script, const string& comment) {
    ...
    if ( info )
        ...
        DbgAndWarn(util::fmt("Lookup of script %s failed for summary comment %s", name.c_str(), comment.c_str()));
}
