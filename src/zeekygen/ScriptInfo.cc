...
static void add_summary_rows(const ODesc& id_desc, const vector<string>& cmnts, ReStructuredTextTable* table) {
    vector<string> row;
...
static string make_summary(const string& heading, char underline, char border, const id_info_list& id_list) {
    if ( id_list.empty() )
        return "";
...
static string make_redef_summary(const string& heading, char underline, char border, const string& from_script,
...
void ScriptInfo::AddIdentifierInfo(IdentifierInfo* info) { id_info[info->Name()] = info; }
    ...
    for ( id_info_map::const_iterator it = id_info.begin(); it != id_info.end(); ++it ) {
        ...
        if ( IsFunc(id->GetType()->Tag()) ) {
            switch ( id->GetType()->AsFuncType()->Flavor() ) {
                ...
                case FUNC_FLAVOR_EVENT:
                    DBG_LOG(DBG_ZEEKYGEN, "Filter id '%s' in '%s' as a event", id->Name(), name.c_str());
                    events.push_back(info);
                    break;
                case FUNC_FLAVOR_FUNCTION:
                    DBG_LOG(DBG_ZEEKYGEN, "Filter id '%s' in '%s' as a function", id->Name(), name.c_str());
        ...
        if ( id->IsConst() ) {
            if ( id->GetAttr(zeek::detail::ATTR_REDEF) ) {
                DBG_LOG(DBG_ZEEKYGEN, "Filter id '%s' in '%s' as a redef_option", id->Name(), name.c_str());
                redef_options.push_back(info);
            }
            else {
                DBG_LOG(DBG_ZEEKYGEN, "Filter id '%s' in '%s' as a constant", id->Name(), name.c_str());
            ...
            DBG_LOG(DBG_ZEEKYGEN, "Filter id '%s' in '%s' as an runtime option", id->Name(), name.c_str());
