#!/usr/bin/env python3
"""Generate ldap-delete.pcap -- an LDAP delete that Zeek fails to parse.
...
LDAP messages, in order:
  ...
  2 -> delRequest for uid=jdoe,ou=People,dc=example,dc=com
  2 <- delResponse, success
  3 -> searchRequest, base dc=example,dc=com, filter (objectClass=*)
  3 <- searchResDone, success
  4 -> unbindRequest

The delRequest is encoded per RFC 4511: DelRequest ::= [APPLICATION 10] LDAPDN is
IMPLICIT-tagged, so the protocolOp is the primitive tag 0x4A followed directly by
the DN octets (0x4a 0x24 "uid=jdoe,..."), with no nested OCTET STRING TLV.

The search and the unbind after the delete are there on purpose: Zeek's violation
...
APP_DEL_REQUEST = 0x4A  # [APPLICATION 10] LDAPDN, primitive (!)
APP_DEL_RESPONSE = 0x6B  # [APPLICATION 11] SEQUENCE, constructed
...
def del_request(message_id, dn):
    # DelRequest ::= [APPLICATION 10] LDAPDN -- IMPLICIT tag over an OCTET
    # STRING, so the content octets are the DN itself, not a nested TLV.
    return ldap_message(message_id, tlv(APP_DEL_REQUEST, dn.encode("utf-8")))


def del_response(message_id, code=0):
...
def main():
    ...
    s.client_data(del_request(2, TARGET_DN))
    s.server_data(del_response(2))
    # Everything below happens after the delete, on the same connection.
    s.client_data(search_request(3, BASE_DN))
