# @TEST-DOC: Test &publish_on_change with TABLE_ELEMENT_NEW and TABLE_ELEMENT_DELETE, deleting just one entry.
#
...
event zeek_init()
	...
	when ( "x" in tbl1 )
		...
		# Wait for manager to wipe the "y" entry in the
		# table once it saw the "y" insert.
		when ( "y" !in tbl1 )
			{
			print "tbl1 without y!", tbl1;
			terminate();
			}
		timeout 10sec
			{
