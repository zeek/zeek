...
export {
	global tbl: table[string] of count &on_change=Cluster::Table::publish_element_new;
}
...
event go()
	{
	tbl[Cluster::node] = Cluster::local_node_type() == Cluster::WORKER ? 42 : 4711;
	}
...
event Cluster::node_up(name: string, id: string) &priority=-5
	...
	if ( |nodes_up| == |Cluster::nodes| - 1 )
		...
		when ( |tbl| >= 4 )
			...
			for ( k in table_keys(tbl) as set[string] )
				skeys += k;
