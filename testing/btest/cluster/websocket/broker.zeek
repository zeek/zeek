# @TEST-DOC: Test WebSockets clients when Broker cluster backend is enabled.
#
...
event Cluster::node_up(name: string, id: string)
	...
	if ( name == "worker-1" )
		...
		# Broker::listen_websocket("127.0.0.1", to_port(getenv("WEBSOCKET_PORT")));
	}
