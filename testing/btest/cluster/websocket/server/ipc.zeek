# @TEST-DOC: Running a standalone Zeek using the ipc:// transport with ZeroMQ.
#
...
# @TEST-EXEC: WEBSOCKET_PORT=${ZEEK_WEBSOCKET_LISTEN_PORT} python3 ./client.py >> out
# @TEST-EXEC: echo "ipc fifos:" >> out
# @TEST-EXEC: ls ./zeek/ipc-* >> out
#
...
# Redef snippet for running XPUB/XSUB on IPC pipes.
@load base/utils/numbers
module Cluster::Backend::ZeroMQ;

redef listen_xsub_endpoint  = "ipc://./ipc-xsub";
redef connect_xpub_endpoint  = listen_xsub_endpoint;
redef listen_xpub_endpoint  = "ipc://./ipc-xpub";
