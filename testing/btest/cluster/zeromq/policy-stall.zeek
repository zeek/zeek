# @TEST-DOC: Manager and proxies publish to the manager topic. They publish so fast that there XPUB socket stalls.
#
...
# Lower high watermarks from 1000 (default) to something much lower to provoke stalls.
redef Cluster::Backend::ZeroMQ::xpub_sndhwm = 20;
redef Cluster::Backend::ZeroMQ::xsub_rcvhwm = 20;

const total_publishes = 200000;
const batch = 100;

function get_zeromq_stalls(): count {
	local ms = Telemetry::collect_metrics("zeek", "cluster_zeromq_xpub_stalls_total");
...
event zeek_done() {
	...
	local stalls = get_zeromq_stalls();
	if ( stalls == 0 )
		print "GOOD: Observed no XPUB stalls on manager";
	else
		print "FAIL: XPUB stalls on manager";
}
# @TEST-END-FILE
...
event zeek_done() {
	...
	local stalls = get_zeromq_stalls();
	if ( stalls > 0 )
		print "GOOD: Observed XPUB stalls";
	else
		print "FAIL: No XPUB stalls";
}
# @TEST-END-FILE
