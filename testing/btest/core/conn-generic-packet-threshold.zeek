...
# @TEST-EXEC: zeek -b -C -r $TRACES/http/get.pcap %INPUT >> out
...
# @TEST-EXEC: zeek -b -C -r $TRACES/http/get.pcap %INPUT ConnThreshold::generic_packet_thresholds+={10} >> out
...
# @TEST-EXEC: zeek -b -C -r $TRACES/http/get.pcap %INPUT ConnThreshold::generic_packet_thresholds={15} >> out
