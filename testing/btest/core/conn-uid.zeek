...
#
# @TEST-EXEC: zeek -b -D -C -r $TRACES/wikipedia.pcap %INPUT >output
# @TEST-EXEC: btest-diff output
#
# Without a seed, they should differ each time:
#
# @TEST-EXEC: unset ZEEK_SEED_FILE && unset BRO_SEED_FILE && zeek -b -C -r $TRACES/wikipedia.pcap %INPUT >output2
