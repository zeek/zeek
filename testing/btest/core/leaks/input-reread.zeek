...
# @TEST-EXEC: $SCRIPTS/wait-for-file zeek/got2 120 || (btest-bg-wait -k 1 && false)
# @TEST-EXEC: cp input2.log input.log
# @TEST-EXEC: $SCRIPTS/wait-for-file zeek/got4 30|| (btest-bg-wait -k 1 && false)
...
# @TEST-EXEC: $SCRIPTS/wait-for-file zeek/got8 30 || (btest-bg-wait -k 1 && false)
