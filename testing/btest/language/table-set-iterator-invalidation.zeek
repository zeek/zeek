...
event zeek_init()
	...
	for ( i in t )
		# Modifying an existing element is not qualified as modifying
		# membership, so this doesn't trigger a warning.
		t[i] = cat(i);
	...
	for ( i in t )
		# Trying to delete a non-existent element within in a loop
		# does not actually modify membership, so does not trigger
		# a warning.
		delete t[0];
