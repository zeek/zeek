...
zeek::detail::IDPtr NanoTimeOpaqueType::id_ptr;
...
bool NanoTimeOpaqueType::CanCastTo(const zeek::Type* t) const {
    std::fprintf(stderr, "CanCastTo %s\n", zeek::obj_desc(t).c_str());
    return t->Tag() == zeek::TYPE_COUNT || t->Tag() == zeek::TYPE_TIME;
}

zeek::ValPtr NanoTimeOpaqueType::DefaultVal() const {
    std::fprintf(stderr, "DefaultVal\n");
    return zeek::make_intrusive<NanoTimeVal>(0);
}
