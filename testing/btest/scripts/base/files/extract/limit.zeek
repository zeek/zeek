# @TEST-EXEC: zeek -b -r $TRACES/ftp/retr.pcap %INPUT max_extract=3000 efname=1
# @TEST-EXEC: btest-diff --binary extract_files/1
# @TEST-EXEC: btest-diff 1.out
# @TEST-EXEC: zeek -b -r $TRACES/ftp/retr.pcap %INPUT max_extract=3000 efname=2 double_it=T
# @TEST-EXEC: btest-diff --binary extract_files/2
# @TEST-EXEC: btest-diff 2.out
# @TEST-EXEC: btest-diff files.log
# @TEST-EXEC: zeek -b -r $TRACES/ftp/retr.pcap %INPUT max_extract=7000 efname=3 unlimit_it=T
