...
# @TEST-EXEC: zeek -b -r $TRACES/pe/pe.pcap %INPUT
# @TEST-EXEC: test -f pe.log
# @TEST-EXEC: rm *log

# @TEST-EXEC: zeek -b -r $TRACES/pe/pe.pcap %INPUT 'Analyzer::disabled_analyzers += { Files::ANALYZER_PE }'
# @TEST-EXEC: test ! -f pe.log
...
# @TEST-EXEC: zeek -b -r $TRACES/pe/pe.pcap %INPUT 'Analyzer::disabled_analyzers += { Files::ANALYZER_PE }' -e 'event zeek_init() { Analyzer::enable_analyzer(Files::ANALYZER_PE); }'
# @TEST-EXEC: test -f pe.log
# @TEST-EXEC: test -f conn.log
