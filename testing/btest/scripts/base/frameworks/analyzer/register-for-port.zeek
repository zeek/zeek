...
# @TEST-EXEC: zeek -b -r ${TRACES}/ssh/ssh-on-port-80.pcap %INPUT dpd_buffer_size=0 base/protocols/conn base/protocols/ssh
# @TEST-EXEC: cat conn.log | zeek-cut service | grep -q ssh
#
# @TEST-EXEC: zeek -b -r ${TRACES}/ssh/ssh-on-port-80.pcap dpd_buffer_size=0 base/protocols/conn base/protocols/ssh
