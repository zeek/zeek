# @TEST-DOC: Basic functionality for storage: opening/closing a PostgreSQL backend, storing/retrieving data, using async methods
#
# @TEST-REQUIRES: have-postgresql
# @TEST-PORT: POSTGRESQL_PORT
#
# @TEST-EXEC: tar -xf ${FILES}/postgresql/postgres-data-no-auth.tar
# @TEST-EXEC: btest-bg-run postgresql run-postgresql-server ${POSTGRESQL_PORT%/tcp} ../postgres-data-no-auth
...
event zeek_init()
	...
	when [opts, key, value] ( local open_res = Storage::Async::open_backend(
	    Storage::STORAGE_BACKEND_POSTGRESQL, opts, string, string) )
		{
