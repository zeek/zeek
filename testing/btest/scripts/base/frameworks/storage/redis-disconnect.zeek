...
event Storage::connection_lost(tag: string, config: any, reason: string) {
	print "Storage::connection_lost", tag, config, reason;
	terminate();
}
...
event zeek_init()
	...
	# Kill the redis server so it'll fire the connection_lost event.
