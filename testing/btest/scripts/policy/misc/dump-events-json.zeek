...
#
# @TEST-EXEC: zeek -b -r $TRACES/smtp.pcap policy/misc/dump-events %INPUT | jq >all-events.json
# @TEST-EXEC: zeek -b -r $TRACES/smtp.pcap policy/misc/dump-events %INPUT DumpEvents::include_args=F | jq >all-events-no-args.json
# @TEST-EXEC: zeek -b -r $TRACES/smtp.pcap policy/misc/dump-events %INPUT DumpEvents::include=/smtp_/ | jq >smtp-events.json
#
# In the following, the Pcap::file_done event contains the full path to the
# processed pcap, so we normalize it out:
# @TEST-EXEC: zeek -b -r $TRACES/smtp.pcap policy/misc/dump-events %INPUT DumpEvents::dump_all_events=T | jq 'select(.event == "Pcap::file_done").args.path = "XXX"' >really-all-events.json
#
