...
#
# XXX: This does not seem to work: We should not use && with something that fails :-)
#      Fix elsewhere as it causes a bunch of churn.
#
# @TEST-REQUIRES: grep -q "^ZEEK_HAVE_OPENSSL_3_0.*true" $BUILD/CMakeCache.txt && test $? != 0

# @TEST-EXEC: zeek -r $TRACES/smtp.trace %INPUT >all-events.log
# @TEST-EXEC: zeek -r $TRACES/smtp.trace %INPUT DumpEvents::include_args=F >all-events-no-args.log
# @TEST-EXEC: zeek -r $TRACES/smtp.trace %INPUT DumpEvents::include=/smtp_/ >smtp-events.log
# @TEST-EXEC: zeek -r $TRACES/smtp.trace %INPUT DumpEvents::dump_all_events=T | grep -v "CPU: interval\|samples: set\|path: string" > really-all-events.log
#
