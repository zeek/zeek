 ...
 */
std::vector<Option> split(std::string& content) {
    ...
    while ( pos < content.size() ) {
        ...
        std::string orig_line = content.substr(pos, nl_pos - pos);
        ...
        result.push_back({.key = key, .value = value, .orig = orig_line});
        pos = nl_pos + 1;
    }
...
CpuList::CpuList(const std::string& list) {
    size_t pos = 0;
    while ( pos < list.size() ) {
        auto next = list.find(',', pos);

        auto cpu = list.substr(pos, next - pos);
        cpus.push_back(std::atoi(cpu.c_str()));
