...
public:
    ...
    std::filesystem::path SpoolDir() const { return ZeekBaseDir() / "spool"; }
     ...
     */
    std::filesystem::path LogArchiveDir() const { return ZeekBaseDir() / "logs"; }
     ...
     * ZeekArgs()
     */
    const std::string ZeekArgs() const {
        // This policy script configures some basics for a systemd based cluster.
        //
        // Make this configurable if there's ever another generator.
        return "policy/misc/zeek-systemd-generator " + args;
    }
