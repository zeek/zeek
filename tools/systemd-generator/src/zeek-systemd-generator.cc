...
Unit systemd_add_node_unit(const path& file, const std::string& node, const std::string& description,
    ...
    // Loggers add the logging spool dir, too.
    unit.AddReadWritePath(config.SpoolDir() / node);
    unit.SetWorkingDirectory(config.SpoolDir() / node);

    // Replaced by worker with SetExecStart() to add the interface.
    unit.AddExecStart(config.ZeekExe(), {config.ZeekArgs()});
...
void systemd_write_units(const path& dir, const ZeekClusterConfig& config) {
    ...
    setup_unit.AddExecStart("mkdir -p " + (config.ZeekBaseDir() / "logs").string());
    setup_unit.AddExecStart("chown " + config.User() + ":" + config.Group() + " " +
                            (config.ZeekBaseDir() / "logs").string());
    ...
    // This makes the whole <PREFIX> read-writeable for the logger
    // process such that it can move logs from its working directory
    // into <PREFIX>/logs. Obviously, uid/gid ownership still exists,
    // so that shouldn't be all that bad. If we were to move
    // <PREFIX>/spool into <PREFIX>/var/spool we could limit
    // read-write path to <PREFIX>/var/ here.
    //
    // We could also mark certain paths read-only.
    logger_unit.AddReadWritePath(config.ZeekBaseDir());
    ...
    worker_unit.SetExecStart(config.ZeekExe(), {"-i", "${INTERFACE}", config.ZeekArgs()});
...
int main(int argc, const char* argv[]) {
    ...
    if ( argc == 2 || argc == 4 ) {
        ...
        std::fprintf(stderr, "Usage: %s normal-dir [early-dir] [late-dir]\n", argv[0]);
        std::exit(1);
    }
    ...
    // CONFIG_FILE environment overrides default search path.
    const char* env_config_file = std::getenv("CONFIG_FILE");
    if ( env_config_file && std::strlen(env_config_file) > 0 ) {
        auto canonical = std::filesystem::weakly_canonical(env_config_file);
    ...
    // If env_config_file was given and we get here, treat it as an error.
    return (env_config_file && std::strlen(env_config_file) > 0) ? 1 : 0;
}
